...
local ActivityPurpEqpUpTableView = class("ActivityPurpEqpUpTableView", TableView)

--
function ActivityPurpEqpUpTableView:ctor(size, activityId)
	ActivityPurpEqpUpTableView.super.ctor(self, size, SCROLL_DIRECTION_VERTICAL)

	self.m_activityId = ACTIVITY_ID_PURPLE_EQP_UP
	self.m_activityContent = ActivityMO.getActivityContentById(self.m_activityId)

	gprint("ActivityPurpEqpUpTableView ctor activity id:", activityId)
	gdump(self.m_activityContent, "ActivityPurpEqpUpTableView:ctor contents")

	self.m_cellSize = cc.size(size.width, 190)
end

function ActivityPurpEqpUpTableView:numberOfCells()
	local num = 0
...
function ActivityPurpEqpUpTableView:cellSizeForIndex(index)
	return self.m_cellSize
end

function ActivityPurpEqpUpTableView:createCellAtIndex(cell, index)
	ActivityPurpEqpUpTableView.super.createCellAtIndex(self, cell, index)
	...
	-- gdump(item, "ActivityPurpEqpUpTableView createCellAtIndex")
	-- gdump(condition, "ActivityPurpEqpUpTableView createCellAtIndex")
	...
	-- 当前进度
	local desc = ui.newTTFLabel({text = condition.param .. CommonText[468] .. ":", font = G_FONT, size = FONT_SIZE_TINY, x = 20, y = 130, align = ui.TEXT_ALIGN_CENTER}):addTo(cell)
...
function ActivityPurpEqpUpTableView:onReceiveCallback(tag, sender)
...
return ActivityPurpEqpUpTableView
