...
public class FightCalc {
	...
	static public boolean isDodge(Force force, Force target) {
        ...
        boolean isDodge = RandomHelper.isHitRangeIn1000(prob);
//        String atkNick = force.fighter.player != null ? force.fighter.player.lord.getNick() : "atk";
//        String defNick = target.fighter.player != null ? target.fighter.player.lord.getNick() : "def";
//        LogUtil.error(String.format("def nick :%s, def.pos :%d, def.dodge :%d, atk :%s, atk.pos :%d, atk.hit :%d, isDodge prob :%d, isDodge :%d", defNick, target.pos, dodge, atkNick, force.pos, hit, prob, isDodge ? 1 : 0));
        return isDodge;
    }
...
//		LogHelper.ERROR_LOGGER.error("is impale:" + force.calcImpale() + "|" + target.calcDefend() + "|" + countFactor + "|" + prob);
		...
		/*if (hitRangeIn100000) {
			...
			if (force.staticTank.getName().equals("位面火箭") && target.staticTank.getName().equals("位面火箭")){
				LogUtil.error("is impale穿刺:force" + force.count + "| " + target.count + "| " + JSON.toJSONString(force.attrData) + "|\n target" + JSON.toJSONString(target.attrData));
			}
	...
	static public long calcHurt(Force force, Force target, float crit) {
		...
		//战术额外增加的伤害百分比
		if (force.tacticsTankSuitConfig != null && restriction > 0) {
			restriction += force.tacticsTankSuitConfig.getAttrRestrict();
		}
		...
		// 调整伤害加成下限为0.2，上限不变
		double addtionValue = ((BASE + attackData.attackF + force.fighter.auraData[force.type - 1].attackF
				+ attackData.demageF + addDemageF - defendData.injuredF - target.fighter.auraData[target.type - 1].injuredF
				+ impaleSub * 80 + frightenSub * 80) / BASE);
		addtionValue = addtionValue < 0.2 ? 0.2 : addtionValue;

		long hurt = (long)( (attackData.attack * addtionValue * force.count * crit) * (1+(restriction/100.0f)));
