...
local RebelAwardDialog = class("RebelAwardDialog", Dialog)

-- tankId: 需要改装的tank
function RebelAwardDialog:ctor(awardKey)
	RebelAwardDialog.super.ctor(self, IMAGE_COMMON .. "bg_dlg_1.png", UI_ENTER_NONE, {scale9Size = cc.size(582, 834)})
	self:size(582,834)
	self.awardKey = awardKey
end

function RebelAwardDialog:onEnter()
	RebelAwardDialog.super.onEnter(self)
	...
	local view = ContentTableView.new(cc.size(490, self:getBg():height()-142),self.kind)
		:addTo(self:getBg()):pos(45,66)
	view:updateUI(PartyBattleMO.getAll(self.awardKey or "rebelRankAward"))
end

function RebelAwardDialog:onExit()
	RebelAwardDialog.super.onExit(self)
end

return RebelAwardDialog
