package com.account.plat.impl.chCjzjtkzzAppstore;
...
@Component
public class ChCjzjtkzzAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chCjzjtkzzAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.cjzjtk.60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.cjzjtk.300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.cjzjtk.980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.cjzjtk.1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.cjzjtk.3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.cjzjtk.6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay chCjzjtkzz_appstore ");
        LOG.error("pay chCjzjtkzz_appstore  content:" + content);
        LOG.error("[开始参数]");
        try {
...
//			int money = MONEY_MAP.get(rechargeId);
            ...
            if (code != 0) {
                LOG.error("chCjzjtkzz_appstore  充值发货失败！！ " + code);
                if (code == 2) {
                    return returnCode(203);
                }
            } else {
                LOG.error("chCjzjtkzz_appstore  充值发货成功！！ " + code);
            }
    ...
    private boolean verifyAccount(String userid, String token) {
        LOG.error("chCjzjtkzz_appstore  开始调用sidInfo接口");
        try {
