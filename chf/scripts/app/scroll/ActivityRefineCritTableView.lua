--
-- Author: Your Name
-- Date: 2017-05-12 11:50:21
-- 淬炼暴击活动

local ActivityRefineCritTableView = class("ActivityRefineCritTableView", TableView)


function ActivityRefineCritTableView:ctor(size,activityId)
	ActivityRefineCritTableView.super.ctor(self, size, SCROLL_DIRECTION_VERTICAL)
	...
	gprint("ActivityEquipUpCritTableView ctor activity id:", activityId)
	gdump(self.m_activityContent, "ActivityEquipUpCritTableView ctor")
...
function ActivityRefineCritTableView:numberOfCells()
	return #self.m_activityContent.items + 1
end

function ActivityRefineCritTableView:cellSizeForIndex(index)
	return self.m_cellSize
end

function ActivityRefineCritTableView:createCellAtIndex(cell, index)
	ActivityRefineCritTableView.super.createCellAtIndex(self, cell,index)
	...
	--如果是第一个则展示活动Item
	if index == 1 then
		local title = ui.newTTFLabel({text = CommonText[1013][2], font = G_FONT, size = FONT_SIZE_SMALL, x = 40, y = titleBg:getContentSize().height / 2, align = ui.TEXT_ALIGN_CENTER}):addTo(titleBg)
		title:setAnchorPoint(cc.p(0, 0.5))

		local itemView = display.newSprite("image/item/t_refineCrit.jpg"):addTo(cell)
		...
		local name = ui.newTTFLabel({text = CommonText[1013][1], font = G_FONT, size = FONT_SIZE_LIMIT, x = itemView:getPositionX(), y = 30, color = COLOR[1], align = ui.TEXT_ALIGN_CENTER}):addTo(cell)
		--去淬炼
		...
		local btn = CellMenuButton.new(normal, selected, disabled, handler(self, self.goRefine))
		btn:setLabel(CommonText[5027])
		cell:addButton(btn, self.m_cellSize.width - 70, 70)
	else
		local item = self.m_activityContent.items[index - 1]
...
--去淬炼
function ActivityRefineCritTableView:goRefine(tag, sender)
	ManagerSound.playNormalButtonSound()
	require("app.view.ComponentView").new(nil, UI_ENTER_FADE_IN_GATE):push()
end

function ActivityRefineCritTableView:onBuyCallback(tag, sender)
	ManagerSound.playNormalButtonSound()
...
return ActivityRefineCritTableView