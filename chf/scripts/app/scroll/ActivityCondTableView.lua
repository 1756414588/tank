...
local ActivityCondTableView = class("ActivityCondTableView", TableView)

--
function ActivityCondTableView:ctor(size, activityId)
	ActivityCondTableView.super.ctor(self, size, SCROLL_DIRECTION_VERTICAL)

	self.m_activityId = activityId
	self.m_activityContent = ActivityMO.getActivityContentById(self.m_activityId)

	gprint("ActivityCondTableView ctor activity id:", activityId)
	gdump(self.m_activityContent, "ActivityCondTableView:ctor contents")

	self.m_cellSize = cc.size(size.width, 190)
end

function ActivityCondTableView:onEnter()
	ActivityCondTableView.super.onEnter(self)
	self.m_activityHandler = Notify.register(LOCLA_ACTIVITY_EVENT, handler(self, self.onActivityUpdate))
end

function ActivityCondTableView:onExit()
	ActivityCondTableView.super.onExit(self)
...
function ActivityCondTableView:onActivityUpdate(event)
	local offset = self:getContentOffset()
	self:reloadData()
	self:setContentOffset(offset)
end

function ActivityCondTableView:numberOfCells()
	local num = 0
...
function ActivityCondTableView:cellSizeForIndex(index)
	return self.m_cellSize
end

function ActivityCondTableView:createCellAtIndex(cell, index)
	ActivityCondTableView.super.createCellAtIndex(self, cell, index)
	...
	gdump(item, "ActivityCondTableView createCellAtIndex")
	gdump(condition, "ActivityCondTableView createCellAtIndex")
	...
	if awards then
		for awardIndex = 1, #awards do
			...
			local name = ui.newTTFLabel({text = resData.name2, font = G_FONT, size = FONT_SIZE_TINY, x = itemView:getPositionX(), y = 30, color = COLOR[resData.quality], align = ui.TEXT_ALIGN_CENTER}):addTo(cell)
		end
	end
...
function ActivityCondTableView:onReceiveCallback(tag, sender)
