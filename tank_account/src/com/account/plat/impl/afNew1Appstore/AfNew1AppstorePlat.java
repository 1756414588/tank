package com.account.plat.impl.afNew1Appstore;
...
@Component
public class AfNew1AppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/afNew1Appstore/", "plat.properties");
    ...
    private boolean verifyAccount(String openid, String token) {
        try {
            LOG.error("afNew1_appstore开始调用sidInfo接口:" + serverUrl);
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        try {
            ...
            if (!APP_ID.equals(params.get("vid"))) {
                LOG.error("afNew1_appstore 充值应用ID不一致！！ ");
                return "ERROR";
            }
            ...
            if (sign.equalsIgnoreCase(originSign)) {
                ...
                if (code != 0) {
                    LOG.error("afNew1_appstore 充值发货失败！！ " + code);
                }
                return "SUCCESS";
            } else {
                LOG.error("afNew1_appstore 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("afNew1_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
