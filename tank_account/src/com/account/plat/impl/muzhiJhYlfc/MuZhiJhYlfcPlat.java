package com.account.plat.impl.muzhiJhYlfc;
...
@Component
public class MuZhiJhYlfcPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/muzhiJhYlfc/", "plat.properties");
        // if (properties != null) {
    ...
    public String order(WebRequest request, String content) {
        LOG.error("pay muzhiJhYlfc order");
...
//			String url = TEST_URL + "/pay/getOrderID";// 测试URL
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        try {
            ...
            if (code != 0) {
                LOG.error("muzhiJhYlfc 充值发货失败！！ " + code);
            }
            return "SUCCESS";
        } catch (Exception e) {
            LOG.error("muzhiJhYlfc 充值异常！！ ");
            e.printStackTrace();
            return "FAIL";
        }
    }
    ...
    private boolean verifyAccount(String userID, String token) {
        LOG.error("muzhiJhYlfc开始调用sidInfo接口");
        // StringBuffer sb = new StringBuffer();
