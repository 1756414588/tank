package com.account.plat.impl.anfanTest;
...
@Component
public class AnfanTestPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/anfanTest/", "plat.properties");
    ...
    public GameError doLogin(DoLoginRq req, DoLoginRs.Builder response) {
        ...
        if (vParam.length != 2 && vParam.length != 3) {
            return GameError.PARAM_ERROR;
        }
        String ucid, uuid;
        AnfanAccount anfanAccount;
        ucid = vParam[0];
        uuid = vParam[1];
        anfanAccount = verifyAccount(ucid, uuid, APP_ID, APP_SECRET);
    ...
    private AnfanAccount verifyAccount(String ucid, String uuid, String appId, String appSecret) {
        try {
            LOG.error("anfanTest开始调用sidInfo接口:" + serverUrl);
            ...
            // 对排序后的参数附加开发商签名密钥
            postdatasb.append("&signKey=" + appSecret);
     ...
     * @param response
     * @return
     * @see com.account.plat.PlatInterface#payBack(org.springframework.web.context.request.WebRequest,
     * java.lang.String, javax.servlet.http.HttpServletResponse)
     */
    @Override
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay anfanTest");
        LOG.error("[接收到的参数]" + content);
        try {
            Map<String, String> params = new HashMap<String, String>();
			...
			/*if (vid.equals(APP_ID_NEW)){
				signstr = signstr + "signKey=" + SIGN_KEY_NEW;
				LOG.error("signstr " + signstr);
			} else */
            if (vid.equals(APP_ID)) {
                signstr = signstr + "signKey=" + SIGN_KEY;
                LOG.error("signstr " + signstr);
            } else {
                LOG.error("anfanTest 充值应用ID不一致！！ vid :" + vid);
                return "ERROR";
            }
            ...
            if (sign.equalsIgnoreCase(originSign)) {
                ...
                if (code != 0) {
                    LOG.error("anfanTest 充值发货失败！！ " + code);
                }
                return "SUCCESS";
            } else {
                LOG.error("anfanTest 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("anfanTest 充值异常！！ ");
            e.printStackTrace();
            return "ERROR";
        }
    }
