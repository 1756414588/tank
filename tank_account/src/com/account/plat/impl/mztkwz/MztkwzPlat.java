package com.account.plat.impl.mztkwz;
...
@Component
public class MztkwzPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/mztkwz/", "plat.properties");
        ...
        try {
            ...
            if (mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                ...
                if (!AppID.equals(game_id)) {
                    LOG.error("mztkwz 充值回调错误！！gameid: " + game_id);
                    return "success";
                }
                ...
                if (payStatus == 0) {
                    ...
                    if (code != 0) {
                        LOG.error("mztkwz 充值发货失败！！ " + code);
                    }
                }
            }
        } catch (Exception e) {
            LOG.error("mztkwz 充值异常！！ ");
            e.printStackTrace();
        }
    ...
    private boolean verifyAccount(String userid, String usename, String sign) {
        LOG.error("mztkwz 开始调用sidInfo接口");
