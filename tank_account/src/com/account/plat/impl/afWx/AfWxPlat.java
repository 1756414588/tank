package com.account.plat.impl.afWx;
...
@Component
public class AfWxPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/afWx/", "plat.properties");
        // if (properties != null) {
...
//				payInfo.platId = params.get("ucid");
                ...
                if (code != 0) {
                    LOG.error("anfan wx 充值发货失败！！ " + code);
                }
                return "SUCCESS";
            } else {
                LOG.error("anfan wx 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("anfan wx 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        try {
            ...
            if (orginSign.equals(sign)) {
                ...
                if (ret != 0) {
                    if (ret == 1) {
                        LOG.error("af weixin 余额不足！！");
                        return "1";
                    } else {
                        LOG.error("af weixin 余额异常！！");
                        return "2";
                    }
                }

                if (!costMoney(openid, openkey, pay_token, pf, pfkey, serialId, cost * 10)) {
                    LOG.error("af weixin 扣费失败！！");
                    return "3";
                }
                ...
                int code = payToGameServer(payInfo);
                if (code != 0) {
                    LOG.error("af weixin 充值发货失败！！ " + code);
                    return "4";
                }
            ...
            LOG.error("af weixin 充值异常！！" + e.getMessage());
            e.printStackTrace();
            return "6";
        }
    }
    ...
    private boolean verifyAccount(String openid, String accessToken) {
        LOG.error("af weixin开始调用sidInfo接口");
    ...
    public String balance(WebRequest request, String content) {
        LOG.error("af weix balance检查账户余额");
    ...
    private int checkAccount(String openid, String openkey, String pay_token, String pf, String pfkey, int amount) throws UnsupportedEncodingException {
        LOG.error("af weixin开始调用check接口");
        // timestamp=*&appid=**&sig=***&openid=**&encode=1
        long timestamp = (System.currentTimeMillis() / 1000L);
