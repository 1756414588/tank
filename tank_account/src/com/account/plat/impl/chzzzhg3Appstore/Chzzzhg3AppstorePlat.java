package com.account.plat.impl.chzzzhg3Appstore;
...
@Component
public class Chzzzhg3AppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chzzzhg3Appstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.zzzhgzjbt.60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.zzzhgzjbt.300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.zzzhgzjbt.980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.zzzhgzjbt.1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.zzzhgzjbt.3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.zzzhgzjbt.6480");
        MONEY_MAP.put(6, 648);
    }

    public int getPlatNo() {  // // 角色与草花老坦克互通
        return 94;
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay chzzzhg3_appstore");
        LOG.error("pay chzzzhg3_appstore content:" + content);
        LOG.error("chzzzhg3_appstore[开始参数]");
        try {
            Map<String, String> params = new HashMap<String, String>();
            Iterator<String> iterator = request.getParameterNames();
            while (iterator.hasNext()) {
                String paramName = iterator.next();
                LOG.error("chzzzhg3_appstore " + paramName + ":" + request.getParameter(paramName));
                params.put(paramName, URLDecoder.decode(request.getParameter(paramName), "UTF-8"));
            }
            LOG.error("chzzzhg3_appstore [结束参数]");
            ...
            LOG.error("chzzzhg3_appstore signstr:" + signstr);
            LOG.error("chzzzhg3_appstore checkSign:" + checkSign);
            LOG.error("chzzzhg3_appstore sign:" + sign);

            if (!sign.equalsIgnoreCase(checkSign)) {
                LOG.error("chzzzhg3_appstore 签名验证失败");
...
//			int money = MONEY_MAP.get(rechargeId);
            ...
            if (code != 0) {
                LOG.error("chzzzhg3_appstore 充值发货失败！！ " + code);
                if (code == 2) {
                    return returnCode(203);
                }
            } else {
                LOG.error("chzzzhg3_appstore 充值发货成功！！ " + code);
            }
            return returnCode(200);
        } catch (Exception e) {
            LOG.error("chzzzhg3_appstore 充值异常:" + e.getMessage());
            e.printStackTrace();
            return returnCode(203);
        }
    }
    ...
    private boolean verifyAccount(String userid, String token) {
        LOG.error("chzzzhg3_appstore 开始调用sidInfo接口");
        try {
            ...
            sb.append("&sign=");
            sb.append(sign);
            LOG.error("chzzzhg3_appstore需要发送到服务器的数据为：" + sb.toString());
            String result = HttpUtils.sentPost(VERIFY_URL, sb.toString());
            LOG.error("chzzzhg3_appstore[响应结果]" + result);

            JSONObject rsp = JSONObject.fromObject(result);
            String code = rsp.getString("code");
            if (!"200".equals(code)) {
                LOG.error("chzzzhg3_appstore 验证失败");
                return false;
            }
            LOG.error("chzzzhg3_appstore 验证成功");
            return true;
        } catch (Exception e) {
            LOG.error("chzzzhg3_appstore 接口返回异常");
            e.printStackTrace();
            return false;
        } finally {
            LOG.error("chzzzhg3_appstore 调用sidInfo接口结束");
        }
