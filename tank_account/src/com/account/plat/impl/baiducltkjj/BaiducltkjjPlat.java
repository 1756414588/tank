package com.account.plat.impl.baiducltkjj;
...
@Component
public class BaiducltkjjPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/baiducltkjj/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        try {
            ...
            if (orginSign.equals(Sign)) {
                ...
                if (code != 0) {
                    LOG.error("baiducltkjj 充值发货失败！！ " + code);
                } else {
                    LOG.error("baiducltkjj 充值发货成功");
                }
                return packResponse(1).toString();
            } else {
            ...
            LOG.error("baiducltkjj 充值发货异常:" + e.getMessage());
            e.printStackTrace();
            return packResponse(1).toString();
        }
    }
    ...
    private boolean verifyAccount(String accessToken) {
        LOG.error("baiducltkjj 开始调用sidInfo接口");
        ...
        try {
            ...
            LOG.error("baiducltkjj 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        }
        ...
        if (code == 1) {
            LOG.error("baiducltkjj 登陆成功:" + accessToken);
            return true;
        } else {
            LOG.error("baiducltkjj 登陆失败:" + code + " 原因:" + resultMsg);
            return false;
        }
    }
