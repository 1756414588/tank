package com.account.plat.impl.mzHsbwz_appstore;
...
@Component
public class MzHsbwzAppstorePlat extends PlatBase {
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/mzHsbwz_appstore/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay mzHsbwz_appstore(战争时代)");
        LOG.error("mzHsbwz_appstore 接收到的参数" + content);
        try {
            String contentParam = request.getParameter("content");
            String sign = request.getParameter("sign");
            contentParam = new String(Base64.decode(contentParam));
            LOG.error("mzHsbwz_appstore 订单参数" + contentParam);
            ...
            if (mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                ...
                if (payStatus == 0) {
                    ...
                    if (code != 0) {
                        LOG.error("mzHsbwz_appstore(战争时代) 充值发货失败！！ " + code);
                    } else {
                        LOG.error("mzHsbwz_appstore(战争时代) 充值发货成功");
                    }
                } else {
                    LOG.error("mzHsbwz_appstore (战争时代) 充值失败，跳过");
                }
            } else {
                LOG.error("mzHsbwz_appstore (战争时代) 签名验证失败");
            }
        } catch (Exception e) {
            LOG.error("mzHsbwz_appstore (战争时代) 充值异常！！ " + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean verifyAccount(String userid, String usename, String sign, String timestamp) {
        LOG.error("mzHsbwz_appstore (战争时代) 开始调用sidInfo接口");
        ...
        try {
            ...
            LOG.error("mzHsbwz_appstore 签名原文" + signSource);
            LOG.error("mzHsbwz_appstore 签名结果" + signGenerate);
            LOG.error("mzHsbwz_appstore 签名传入" + sign);

            if (sign.equals(signGenerate)) {
                return true;
            }
        } catch (UnsupportedEncodingException e) {
            LOG.error("mzHsbwz_appstore (战争时代) 签名异常" + e.getMessage());
            e.printStackTrace();
        }
