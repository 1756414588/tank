...
local IosUpdateDialog = class("IosUpdateDialog", function()
...
function IosUpdateDialog.getInstance()
	if not instance_ then
		instance_ = IosUpdateDialog.new()
	end
	return instance_
end

function IosUpdateDialog:ctor()
...
function IosUpdateDialog:show(msgData, updateType, callback, cancelCallback)
	...
	gdump(msgData, "IosUpdateDialog")

	local parent = self:getParent()

	if parent then
		gprint("[IosUpdateDialog] dialog added to parent already!!!")
	else
		...
		local msg = ui.newTTFLabel({text = "", font = FONTS, size = FONT_SIZE_MEDIUM - 4, x = self.m_bg:getContentSize().width / 2, y = 240, color = cc.c3b(255, 255, 255)}):addTo(self.m_bg, 2)
		...
		if updateType == 1 then
			...
			self.m_okBtn:setLabel(LoginText[60][2])
			self.m_okBtn:setPosition(self.m_bg:getContentSize().width / 2, 80)
			...
			self.m_cancelBtn:setLabel(LoginText[60][1])
			self.m_cancelBtn:setPosition(self.m_bg:getContentSize().width / 2 - 130, 80)
			...
			self.m_okBtn = MenuButton.new(normal, selected, disabled, onCloseCallback)  -- 确定
			self.m_okBtn:setLabel(LoginText[60][2])
			self.m_okBtn:setPosition(self.m_bg:getContentSize().width / 2 + 130, 80)
			self.m_bg:addChild(self.m_okBtn)
		end
...
function IosUpdateDialog:onExit()
	instance_ = nil
end

return IosUpdateDialog
