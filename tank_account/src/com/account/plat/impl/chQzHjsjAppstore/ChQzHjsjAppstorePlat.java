package com.account.plat.impl.chQzHjsjAppstore;
...
@Component
public class ChQzHjsjAppstorePlat extends PlatBase {
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chQzHjsjAppstore/", "plat.properties");
    ...
    private boolean verifyAccount(String[] param) {
        try {
            LOG.error("ChQzHjsjAppstore 开始调用sidInfo接口");
            ...
            if (LOGIN_STATUS_VALUE_SUCCESS.equals(status)) {
                LOG.error("ChQzHjsjAppstore登陆成功 ");
                return true;
            } else if (LOGIN_STATUS_VALUE_NOLOGIN.equals(status)) {
                LOG.error("ChQzHjsjAppstore登陆失败,用户未登录:" + result);
                return false;
            } else if (LOGIN_STATUS_VALUE_ERROR.equals(status)) {
                LOG.error("ChQzHjsjAppstore登陆失败,参数错误:" + result);
                return false;
            } else {
                LOG.error("ChQzHjsjAppstore登陆失败,未知错误:" + result);
                return false;
            }
        } catch (Exception e) {
            LOG.error("ChQzHjsjAppstore 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("ChQzHjsjAppstore payBack开始");
        try {
            ...
            if (sign.equalsIgnoreCase(checkSign)) {
                ...
                if (code != 0) {
                    LOG.error("ChQzHjsjAppstore 充值发货失败！！ " + code);
                }
                LOG.error("ChQzHjsjAppstore 充值发货成功！！ " + code);
                return "success";

            } else {
                // afNewMjdzh1_appstore 签名不一致！！ 2c461a02105b45922989a515017aa7f0|286cfafa5a9324f2a80518b60a205b6e
                LOG.error("ChQzHjsjAppstore 签名不一致！！ " + checkSign + "|" + sign);
                return "errorSign";
            }
        } catch (Exception e) {
            LOG.error("ChQzHjsjAppstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "error";
        }
    }
    ...
    public static void main(String[] args) {
        ...
        /*LOG.error("1_55500010000001_20180117143530_1_".length());
        try {
        String teststr = "orderid=1516170932173770002&username=wtwl2008&appid=10127&roleid="+URLEncoder.encode("多拉史帝文","UTF-8")+"&serverid="+URLEncoder.encode("枪林弹雨[金山云]","UTF-8")+"&amount=600&paytime=1516170932&attach=01_20180117143530_1&productname="+ URLEncoder.encode("6元60金币","UTF-8") +"&appkey=0fe202ef3cdf9b2f7cf61fbc9b366b5a";
        String sgin  = "3d6d9c0b5c21b88d1fedcb4be327a2a1";
        
       
      
        //  teststr = URLEncoder.encode(teststr,"UTF-8");
            LOG.error(teststr);
            String checkSign = MD5.md5Digest(teststr);
            
            //sgin = URLDecoder.decode(sgin, "UTF-8");
