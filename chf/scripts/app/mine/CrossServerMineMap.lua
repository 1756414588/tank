...
local CrossServerMineLayer = require("app.mine.CrossServerMineLayer")

local CrossServerMineMap = class("CrossServerMineMap",function(size)
...
function CrossServerMineMap:ctor(size)
...
function CrossServerMineMap:onEnter()
    ...
    self.m_plunderHandler = Notify.register(LOCAL_CROSS_PLUNDER_UPDATE_EVENT, handler(self, self.onPlunderUpdate))
    self.m_militaryAreaHandler = Notify.register(LOCAL_CROSS_MILITARY_AREA_UPDATE_EVENT, handler(self, self.onMilitaryAreaUpdate))
    self.m_armyHandler = Notify.register(LOCAL_ARMY_EVENT, handler(self, self.onMilitaryAreaUpdate))

    local function refresh()
        if UiDirector.getTopUiName() == "HomeView3" then
...
function CrossServerMineMap:onExit()
...
function CrossServerMineMap:onMilitaryAreaUpdate(event)
    ...
    StaffBO.asynGetCrossSeniorMap(doneCallback)
end

function CrossServerMineMap:buildWithMapInfo(mapInfo)
    self.layerContainer_ = nil

    self.mapSize_ = cc.size(40, 40)
    self.tileSize_ = cc.size(WORLD_TILE_WIDTH, WORLD_TILE_HEIGHT)

    local child = CrossServerMineLayer.new(self):addTo(self, 1, 1)
    ...
    if not StaffBO.IsCrossServerMineAreaOpen() then  -- 活动时间未到
    ...
    local function gotoNormalServerMineMap()
        UiDirector.pop()
    ...
    --军事矿区
    local normal = display.newSprite(IMAGE_COMMON .. "btn_normal_mine.png")
    local selected = display.newSprite(IMAGE_COMMON .. "btn_normal_mine.png")
    local socialBtn = MenuButton.new(normal, selected, nil, gotoNormalServerMineMap):addTo(self, 2)
    ...
    local function gotoDetail(tag, sender)
        ...
        DetailTextDialog.new(DetailText.crossmilitaryArea):push()
    end
    ...
    local function gotoRank(tag, sender)
        ManagerSound.playNormalButtonSound()
        require("app.view.CrossServerMineRankView").new():push()
    end
...
function CrossServerMineMap:onPlunderUpdate(event)
...
function CrossServerMineMap:locate(x, y, feedback)
    if x < 0 then x = 0 end
...
function CrossServerMineMap:getViewRect()
...
function CrossServerMineMap:loadMap()
    if not self.m_tileUsed then return end

    for x, datas in pairs(self.m_tileUsed) do
        for y, tilePos in pairs(datas) do
            if x >= 0 and x < CROSS_SERVER_MILITARY_AREA_SIZE_WIDTH and y >= 0 and y < CROSS_SERVER_MILITARY_AREA_SIZE_HEIGHT then -- 地图范围内的
                local pos = cc.p(x, y)
...
function CrossServerMineMap:setContentOffset(offset)
    local newPos = offset
    ...
    if newCenterTilePos.x < 0 or newCenterTilePos.x > (CROSS_SERVER_MILITARY_AREA_SIZE_WIDTH - 1)
        or newCenterTilePos.y < 0 or newCenterTilePos.y > (CROSS_SERVER_MILITARY_AREA_SIZE_HEIGHT - 1) then
        Toast.clear()
...
function CrossServerMineMap:onTouch(event)
...
-- point: id, x, y
function CrossServerMineMap:addTouchPoint(point)
    local find = nil
...
function CrossServerMineMap:deleteTouchPoint(point)
    local findPos = nil
    for index = 1, #self.m_touches do
        local touch = self.m_touches[index]
        if touch.id == point.id then
...
function CrossServerMineMap:onTouchBegan(event)
    if not self:isVisible() then return false end
...
function CrossServerMineMap:onTouchAdded(event)
...
function CrossServerMineMap:onTouchMoved(event)
    if not self:isVisible() then return end
...
function CrossServerMineMap:onTouchRemoved(event)
    if not self:isVisible() then return end
...
function CrossServerMineMap:onTouchEnded(event)
    if not self:isVisible() then return end
...
function CrossServerMineMap:onTouchCancelled(event)
    if not self:isVisible() then return end
...
function CrossServerMineMap:tileHighlight(tilePos)
...
function CrossServerMineMap:tileUnhighlight(tilePos)
    -- self.m_touchedTilePos
end

function CrossServerMineMap:tileTouched(tilePos)
    ManagerSound.playNormalButtonSound()
    ...
    local function click()
        local CrossMineDialog = require("app.dialog.CrossMineDialog")
        CrossMineDialog.new(tilePos.x, tilePos.y):push()
    end
...
function CrossServerMineMap:showCenter()
