package com.account.plat.impl.mzHjxd_appstore;
...
@Component
public class MzHjxdAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/mzHjxd_appstore/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay mzHjxd_appstore content:" + content);
        boolean flag = false;
        ...
        try {
            if (flag) {// 拇指充值
                ...
                LOG.error("mzHjxd_appstore content " + body);
                LOG.error("mzHjxd_appstore 签名结果 " + mysign + "|" + sign);
                if (!mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                    LOG.error("mzHjxd_appstore 签名失败");
                    return "fail";
                }
                ...
                if (payStatus != 0) {
                    LOG.error("mzHjxd_appstore 充值未完成");
                    return "fail";
                }
                ...
                if (info.length < 3) {
                    LOG.error("mzHjxd_appstore 自由参数不正确");
                    return "success";
                }
                ...
                if (code != 0) {
                    LOG.error("mzHjxd_appstore 充值发货失败！！ " + code);
                } else {
                    LOG.error("mzHjxd_appstore 充值发货成功！！ ");
                }
                return "success";
                ...
                LOG.error("mzHjxd_appstore appstore 返回" + result);
                ...
                if (!product_id.equals(RECHARGE_MAP.get(rechargeId))) {
                    LOG.error("mzHjxd_appstore rechargeId abnormal!!!");
                    return "FAILURE";
                }
            ...
            LOG.error("mzHjxd_appstore 充值异常！！ ");
            e.printStackTrace();
            return "0|" + (System.currentTimeMillis() / 1000);
        }
    }
    ...
    private boolean verifyAccount(String userId, String usename, String sign, String time) {
        LOG.error("mzHjxd_appstore 开始调用sidInfo接口");
        try {
            ...
            LOG.error("mzHjxd_appstore 签名原文" + signSource);
            LOG.error("mzHjxd_appstore " + signGenerate);
            LOG.error("mzHjxd_appstore 签名传入" + sign);
