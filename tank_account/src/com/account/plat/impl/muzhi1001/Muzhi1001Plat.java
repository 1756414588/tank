package com.account.plat.impl.muzhi1001;
...
@Component
public class Muzhi1001Plat extends PlatBase {
    // sdk server的接口地址
...
// 分配给游戏合作商的接入密钥,请做好安全保密
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/muzhi1001/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay muzhi1001");
        LOG.error("pay muzhi1001 [接收到的参数]" + content);
        try {
            String contentParam = request.getParameter("content");
            String sign = request.getParameter("sign");
            contentParam = new String(Base64.decode(contentParam));
            LOG.error("pay muzhi1001 [content]" + contentParam);
            ...
            if (mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                ...
                LOG.error("pay muzhi1001 [订单参数]" + json);
                ...
                if (!AppID.equals(game_id)) {
                    LOG.error("pay muzhi1001 充值回调错误！！gameid: " + game_id);
                    return "success";
                }
                ...
                if (payStatus == 0) {
                    ...
                    if (code != 0) {
                        LOG.error("pay muzhi1001 充值发货失败！！ " + code);
                    } else {
                        LOG.error("pay muzhi1001 充值,发货成功");
                    }
                } else {
                    LOG.error("pay muzhi1001 充值,失败订单，跳过");
                }
            } else {
                LOG.error("pay muzhi1001 充值,签名验证失败 ");
            }
        } catch (Exception e) {
            LOG.error("pay muzhi1001 充值异常！！ ");
            LOG.error("pay muzhi1001 充值异常:" + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean verifySgin(String signSource, String sign) {
        ...
        try {
            ...
            LOG.error("pay muzhi1001[签名原文]" + signSource);
            LOG.error("pay muzhi1001[签名结果]" + signGenerate);
            LOG.error("pay muzhi1001[签名传入]" + sign);
            return sign.equals(signGenerate);
        } catch (UnsupportedEncodingException e) {
            LOG.error("pay muzhi1001 用户验证异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private boolean verifyAccount(String userid, String usename, String sign, String timestamp) {
        ...
        LOG.error("pay muzhi1001 开始调用新的sidInfo接口");
    ...
    public static void main(String[] args) {
        ...
        String sign = vParam[2];
        String timestamp = vParam.length == 4 ? vParam[3] : null;
        new Muzhi1001Plat().verifySgin(userId + "zty337" + timestamp, sign);
    }
}
