package com.account.plat.impl.mzTksjdz_appstore;
...
@Component
public class MzTksjdzAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/mzTksjdz_appstore/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay mzTksjdz_appstore content:" + content);
        boolean flag = false;
        ...
        try {
            if (flag) {// 拇指充值
                ...
                LOG.error("mzTksjdz_appstore content " + body);
                LOG.error("mzTksjdz_appstore 签名结果 " + mysign + "|" + sign);
                ...
                if (payStatus != 0) {
                    LOG.error("mzTksjdz_appstore 充值未完成");
                    return "fail";
                }
                ...
                if (info.length < 3) {
                    LOG.error("mzTksjdz_appstore 自由参数不正确");
                    return "success";
                }
                ...
                if (code != 0) {
                    LOG.error("mzTksjdz_appstore 充值发货失败！！ " + code);
                } else {
                    LOG.error("mzTksjdz_appstore 充值发货成功！！ ");
                }
                return "success";
                ...
                LOG.error("mzTksjdz_appstore appstore 返回" + result);
                ...
                if (!product_id.equals(RECHARGE_MAP.get(rechargeId))) {
                    LOG.error("mzTksjdz_appstore rechargeId abnormal!!!");
                    return "FAILURE";
                }
            ...
            LOG.error("mzTksjdz_appstore 充值异常！！ ");
            e.printStackTrace();
            return "0|" + (System.currentTimeMillis() / 1000);
        }
    }
    ...
    private boolean verifyAccount(String userId, String usename, String sign, String time) {
        LOG.error("mzTksjdz_appstore 开始调用sidInfo接口");
        try {
            ...
            LOG.error("mzTksjdz_appstore 签名原文" + signSource);
            LOG.error("mzTksjdz_appstore " + signGenerate);
            LOG.error("mzTksjdz_appstore 签名传入" + sign);
