package com.account.plat.impl.mzjttx_appstore;
 ...
 */
@Component
public class MzzjTtxAppstorePlat extends PlatBase {
   ...
   // private static String AppID;
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/mzjttx_appstore/", "plat.properties");
        ...
        try {
            ...
            if (mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                ...
                if (payStatus == 0) {
                    ...
                    if (code != 0) {
                        LOG.error("MzzjTtx(军团突袭) 充值发货失败！！ " + code);
                    } else {
                        LOG.error("MzzjTtx(军团突袭) 充值发货成功");
                    }
                } else {
                    LOG.error("MzzjTtx (军团突袭) 充值失败，跳过");
                }
            } else {
                LOG.error("MzzjTtx (军团突袭) 签名验证失败");
            }
        } catch (Exception e) {
            LOG.error("MzzjTtx (军团突袭) 充值异常！！ " + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean verifyAccount(String userid, String usename, String sign, String timestamp) {
        LOG.error("MzzjTtx (军团突袭) 开始调用sidInfo接口");
        ...
        try {
            ...
            LOG.error("MzzjTtx 签名原文" + signSource);
            LOG.error("MzzjTtx 签名结果" + signGenerate);
            LOG.error("MzzjTtx 签名传入" + sign);

            if (sign.equals(signGenerate)) {
                return true;
            }
        } catch (UnsupportedEncodingException e) {
            LOG.error("MzzjTtx (军团突袭) 签名异常", e);
        }
