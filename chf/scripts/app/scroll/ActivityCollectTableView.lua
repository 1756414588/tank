...
local ActivityCollectTableView = class("ActivityCollectTableView", TableView)

--
function ActivityCollectTableView:ctor(size, activityId)
	ActivityCollectTableView.super.ctor(self, size, SCROLL_DIRECTION_VERTICAL)

	self.m_activityId = ACTIVITY_ID_RESOURCE
	self.m_activityContent = ActivityMO.getActivityContentById(self.m_activityId)

	gprint("ActivityCollectTableView ctor activity id:", activityId)
	gdump(self.m_activityContent, "ActivityCollectTableView ctor")

	self.m_cellSize = cc.size(size.width, 190)
end

function ActivityCollectTableView:numberOfCells()
	return ((#self.m_activityContent.items) * 4)
end

function ActivityCollectTableView:cellSizeForIndex(index)
	return self.m_cellSize
end

function ActivityCollectTableView:createCellAtIndex(cell, index)
	ActivityCollectTableView.super.createCellAtIndex(self, cell, index)

	local resId = index % 5 + 1
	local condIndex = math.ceil(index / 5)

	local activity = ActivityMO.getActivityById(self.m_activityId)
	local item = self.m_activityContent.items[resId]
	gdump(item, "ActivityCollectTableView:createCellAtIndex")
	-- print("resId:", resId)
	...
	-- 当前进度
	local desc = ui.newTTFLabel({text = CommonText[446] .. ":", font = G_FONT, size = FONT_SIZE_TINY, x = 20, y = 130, align = ui.TEXT_ALIGN_CENTER}):addTo(cell)
...
function ActivityCollectTableView:onReceiveCallback(tag, sender)
...
return ActivityCollectTableView
