...
local BusErrorDialog = class("BusErrorDialog", function()
...
function BusErrorDialog.getInstance()
	if not instance_ then
		instance_ = BusErrorDialog.new()
	end
	return instance_
end

function BusErrorDialog:ctor()
...
function BusErrorDialog:show(msgData, callback)
	...
	gdump(msgData, "BusErrorDialog")

	local parent = self:getParent()

	if parent then
		gprint("[BusErrorDialog] dialog added to parent already!!!")
	else
		...
		local msg = ui.newTTFLabel({text = "", font = FONTS, size = FONT_SIZE_SMALL, x = self.m_bg:getContentSize().width / 2, y = 240, color = cc.c3b(255, 255, 255)}):addTo(self.m_bg, 2)
		if msgData.code and ErrorText["text" .. tostring(msgData.code)] then
			msg:setString(ErrorText["text" .. tostring(msgData.code)])
...
function BusErrorDialog:onExit()
	instance_ = nil
end

return BusErrorDialog
