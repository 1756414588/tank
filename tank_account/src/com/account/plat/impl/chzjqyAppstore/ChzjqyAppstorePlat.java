package com.account.plat.impl.chzjqyAppstore;
...
@Component
public class ChzjqyAppstorePlat extends PlatBase {
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chzjqyAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.zjqy.60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.zjqy.300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.zjqy.980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.zjqy.1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.zjqy.3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.zjqy.6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    public String payBack(WebRequest request, String content,
        ...
        LOG.error("pay chzjqy_appstore");
        LOG.error("pay chzjqy_appstore content:" + content);
        LOG.error("[开始参数]");
        try {
...
//			int money = MONEY_MAP.get(rechargeId);
            ...
            if (code != 0) {
                ...
                LOG.error("chzjqy_appstore 充值发货失败！！ " + code);
            } else {
                LOG.error("chzjqy_appstore 充值发货成功！！ " + code);
            }
            return returnCode(200);
        } catch (Exception e) {
            LOG.error("chzjqy_appstore 充值异常:" + e.getMessage());
            e.printStackTrace();
            return returnCode(203);
        }
    }
    ...
    private boolean verifyAccount(String userid, String token) {
        LOG.error("chzjqy_appstore 开始调用sidInfo接口");
        try {
