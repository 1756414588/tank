...
function ShopTableView:updateCell(cell, index)
	...
	local left = data.maxNumber - (self.data[data.treasureId] and self.data[data.treasureId] or 0)
	local t = UiUtil.label(CommonText[883][2]):addTo(cell)
		:align(display.LEFT_CENTER,320,114)
	UiUtil.label("("..left .."/".. data.maxNumber..")",nil,COLOR[2]):addTo(cell):rightTo(t)
	...
	-- 名称
	local name = ui.newTTFLabel({text = data.treasureName, font = G_FONT, size = FONT_SIZE_SMALL, x = 170, y = 114, color = COLOR[data.color]}):addTo(cell)
	...
	local view = UiUtil.createItemSprite(ITEM_KIND_HUANGBAO):addTo(cell)
	...
	local count = UserMO.getResource(ITEM_KIND_HUANGBAO)
...
local ShopHuangbaoView = class("ShopHuangbaoView", UiNode)

function ShopHuangbaoView:ctor(uiEnter)
	ShopHuangbaoView.super.ctor(self, "image/common/bg_ui.jpg")
end

function ShopHuangbaoView:onEnter()
	ShopHuangbaoView.super.onEnter(self)
	...
	local view = ShopTableView.new(cc.size(self:getBg():getContentSize().width, self:getBg():getContentSize().height - 187 - 45), VIEW_FOR_FIGHT):addTo(self:getBg())
...
function ShopHuangbaoView:updateView()
	CombatBO.GetTreasureShop(function(week,list)
			self.m_tableView.m_propIds = CombatMO.queryShopByWeek(week)
			self.m_tableView.data = {}
			for k,v in ipairs(list) do
				self.m_tableView.data[v.treasureId] = v.buyNum
			end
			self.m_tableView:reloadData()
		end)
end

return ShopHuangbaoView
