package com.account.plat.impl.muzhiEn;
...
@Component
public class MuzhiEnPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/muzhiEn/", "plat.properties");
        ...
        try {
            ...
            if (mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                ...
                if (!AppID.equals(game_id)) {
                    LOG.error("muzhiEn 充值回调错误！！gameid: " + game_id);
                    return "success";
                }
                ...
                if (payStatus == 0) {
                    ...
                    if (rechargeId > 0 && PRICE_MAP.get(price).intValue() != MONEY_MAP.get(rechargeId).intValue()) {
                        LOG.error("muzhiEn 商品金额不匹配 充值发货失败  ！！ ");
                        return "fail";
                    }
                    ...
                    if (code != 0) {
                        LOG.error("muzhiEn 充值发货失败！！ " + code);
                    } else {
                        LOG.error("muzhiEn 充值,发货成功");
                    }
                } else {
                    LOG.error("muzhiEn 充值,失败订单，跳过");
                }
            } else {
                LOG.error("muzhiEn 充值,签名验证失败 ");
            }
        } catch (Exception e) {
            LOG.error("muzhiEn 充值异常！！ ");
            LOG.error("muzhiEn 充值异常:" + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean verifyAccount(String userid, String usename, String sign) {
        LOG.error("muzhiEn 开始调用sidInfo接口");
        ...
        try {
            ...
            LOG.error("muzhiEn 用户验证异常:" + e.getMessage());
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
		...
		// String signSource = usename + AppKey;// 组装签名原文
		// String signGenerate = Rsa.getMD5(signSource).toLowerCase();
		String contentParam = "";
		contentParam = new String(Base64.decode(contentParam));
		LOG.error(contentParam);
		// try {
		// String signSource = "@# linilq,&zty337";
		// signSource = URLEncoder.encode(signSource, "utf-8");
		// String signGenerate = Rsa.getMD5(signSource).toLowerCase();
		//
		// LOG.error("[签名原文]" + signSource);
