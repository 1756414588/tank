package com.account.plat.impl.caohuanew;
...
@Component
public class CaohuanewPlat extends PlatBase {
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/caohuanew/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content,
        ...
        LOG.error("pay caohuanew");
        LOG.error("pay caohuanew content:" + content);
        LOG.error("[开始参数]");
        try {
...
//			}
            ...
            if (code != 0) {
                ...
                LOG.error("caohuanew 充值发货失败！！ " + code);
            } else {
                LOG.error("caohuanew 充值发货成功！！ " + code);
            }
            return returnCode(200);
        } catch (Exception e) {
            LOG.error("caohuanew 充值异常:" + e.getMessage());
            e.printStackTrace();
            return returnCode(203);
        }
    }
    ...
    private boolean verifyAccount(String userid, String token) {
        LOG.error("caohuanew 开始调用sidInfo接口");
        try {
