package com.account.plat.impl.chxsjtAppstore;
...
@Component
public class ChxsjtAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chxsjtAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.xsjt.60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.xsjt.300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.xsjt.980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.xsjt.1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.xsjt.3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.xsjt.6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    public GameError doLogin(DoLoginRq req, DoLoginRs.Builder response) {
        ...
        if (vParam.length != 4 && vParam.length != 2) {
            return GameError.PARAM_ERROR;
        }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        LOG.error("pay chxsjt_appstore");
        LOG.error("pay chxsjt_appstore content:" + content);
        ...
        try {
            ...
            if (code != 0) {
                LOG.error("chxsjt_appstore 充值发货失败！！ " + code);
                return "0|" + (System.currentTimeMillis() / 1000);
            }

            return "1|" + (System.currentTimeMillis() / 1000);
        } catch (Exception e) {
            LOG.error("chxsjt_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "0|" + (System.currentTimeMillis() / 1000);
        }
    }
...
//			int money = MONEY_MAP.get(rechargeId);
    ...
    private boolean verifyAccount(String[] param) {
        LOG.error("chxsjt_appstore 开始调用sidInfo接口");
        ...
        try {
            if (sign.equals(param[2])) {// 成功
                LOG.error("chxsjt_appstore 登陆成功");
                return true;
            } else {
                LOG.error("chxsjt_appstore 登陆失败");
                return false;
            }
        } catch (Exception e) {
            LOG.error("chxsjt_appstore 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
