package com.account.plat.impl.afTqdknHDAppstore;
...
@Component
public class AfTqdknHDAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/afTqdknHDAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.dg.tqdknhd60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.dg.tqdknhd300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.dg.tqdknhd980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.dg.tqdknhd1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.dg.tqdknhd3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.dg.tqdknhd6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    private AnfanAccount verifyAccount(String uid, String ucid, String uuid) {
        try {
            LOG.error("afTqdknHD_appstore 开始调用sidInfo接口:" + serverUrl);
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        // TODO Auto-generated method stub
        LOG.error("pay afTqdknHD_appstore");
        LOG.error("[接收到的参数]" + content);
        try {
            Map<String, String> params = new HashMap<String, String>();
            ...
            if (!APP_ID.equals(params.get("vid"))) {
                LOG.error("afTqdknHD_appstore 充值应用ID不一致！！ ");
                return "ERROR";
            }
            ...
            if (sign.equalsIgnoreCase(originSign)) {
                ...
                if (code != 0) {
                    LOG.error("afTqdknHD_appstore 充值发货失败！！ " + code);
                }
                return "SUCCESS";
            } else {
                LOG.error("afTqdknHD_appstore 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("afTqdknHD_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
