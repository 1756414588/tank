...
local MilitaryMineDialog = class("MilitaryMineDialog", Dialog)

function MilitaryMineDialog:ctor(x, y)
	MilitaryMineDialog.super.ctor(self, IMAGE_COMMON .. "bg_dlg_1.png", UI_ENTER_NONE, {scale9Size = cc.size(588, 400)})
	
	gprint("MilitaryMineDialog:ctor ==> x:", x, "y:", y)
	self.m_x = x
	self.m_y = y
end

function MilitaryMineDialog:onEnter()
	MilitaryMineDialog.super.onEnter(self)
	...
	local mine = StaffBO.getMineAt(cc.p(self.m_x, self.m_y))
	self.m_mine = mine
	...
	local mapData = StaffMO.getMapDataAt(self.m_x, self.m_y)
...
function MilitaryMineDialog:onScoutCallback(tag, sender)
	...
	ConfirmDialog.new(string.format(CommonText[310], UiUtil.strNumSimplify(scout.scoutCost), str, UserMO.scout_ + 1), function()
			...
			StaffBO.asynSctSeniorMine(doneCallback, self.m_x, self.m_y)
		end):push()
end

function MilitaryMineDialog:onPlunderCallback(tag, sender)
	ManagerSound.playNormalButtonSound()
    ...
    StaffMO.curAttackPos_ = cc.p(self.m_x, self.m_y)
...
function MilitaryMineDialog:onAttackCallback(tag, sender)
	ManagerSound.playNormalButtonSound()
