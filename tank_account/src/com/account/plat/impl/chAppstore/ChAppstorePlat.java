package com.account.plat.impl.chAppstore;
...
@Component
public class ChAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.caohua.tank60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.caohua.tank300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.caohua.tank980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.caohua.tank1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.caohua.tank3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.caohua.tank6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    public GameError doLogin(DoLoginRq req, DoLoginRs.Builder response) {
        ...
        if (account == null) {
            ...
            zhtService.checkZhtIdfa(account); // 检测智汇推激活
            wxAdService.checkWxIdfa(account); // 检测微信广告激活
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        LOG.error("pay ch appstore");
        LOG.error("pay ch appstore content:" + content);
...
//			int money = MONEY_MAP.get(rechargeId);
            ...
            if (code != 0) {
                ...
                LOG.error("ch appstore 充值发货成功！！ " + code);
            }
    ...
    private boolean verifyAccount(String[] param) {
        LOG.error("ch appstore 开始调用sidInfo接口");
        ...
        try {
            if (sign.equals(param[2])) {// 成功
                LOG.error("ch appstore 登陆成功");
                return true;
            } else {
                LOG.error("ch appstore 登陆失败");
                return false;
            }
        } catch (Exception e) {
            // TODO: handle exception
            LOG.error("ch appstore 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
