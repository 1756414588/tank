...
local ComponentWarehouseView = class("ComponentWarehouseView", UiNode)

function ComponentWarehouseView:ctor(viewFor)
	ComponentWarehouseView.super.ctor(self, "image/common/bg_ui.jpg", UI_ENTER_NONE)
end

function ComponentWarehouseView:onEnter()
	ComponentWarehouseView.super.onEnter(self)

	-- 配件仓库
	self:setTitle(CommonText[11] .. CommonText[169])
	...
	--  "配件", "碎片", "材料"
	local pages = {CommonText[11], CommonText[164], CommonText[165]}
...
function ComponentWarehouseView:onExit()
...
function ComponentWarehouseView:onPartUpdate(event)
	self.m_pageView:reloadContainer(self.m_pageView:getPageIndex())
end

-- 
function ComponentWarehouseView:showComponent(container)
	local ComponentWarehouseTableView = require("app.scroll.ComponentWarehouseTableView")
	local view = ComponentWarehouseTableView.new(cc.size(container:getContentSize().width, container:getContentSize().height - 160 - 4), VIEW_FOR_PART):addTo(container)
	...
	local desc = ui.newTTFLabel({text = CommonText[167], font = G_FONT, size = FONT_SIZE_SMALL, x = 230, y = btn:getPositionY(), color = COLOR[11], align = ui.TEXT_ALIGN_CENTER}):addTo(container)
	desc:setAnchorPoint(cc.p(0, 0.5))
end

function ComponentWarehouseView:showPiece(container)
	local ComponentWarehouseTableView = require("app.scroll.ComponentWarehouseTableView")
	local view = ComponentWarehouseTableView.new(cc.size(container:getContentSize().width, container:getContentSize().height - 160 - 4), VIEW_FOR_CHIP):addTo(container)
	view:setPosition(0, 160)
	view:reloadData()
...
function ComponentWarehouseView:showMaterial(container)
	local PartMaterialTableView = require("app.scroll.PartMaterialTableView")
	local view = PartMaterialTableView.new(cc.size(container:getContentSize().width, container:getContentSize().height - 4)):addTo(container)
	view:setPosition(0, 0)
	view:reloadData()
end

function ComponentWarehouseView:onBatchCallback(tag, sender)
	ManagerSound.playNormalButtonSound()
	if sender.index == 1 then
		require("app.dialog.BatchDecomposeDialog").new(BATCH_DIALOG_FOR_COMPONENT):push()
	else
		require("app.dialog.BatchDecomposeDialog").new(BATCH_DIALOG_FOR_PIECE):push()
	end
end

return ComponentWarehouseView
