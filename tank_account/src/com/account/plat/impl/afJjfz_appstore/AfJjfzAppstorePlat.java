package com.account.plat.impl.afJjfz_appstore;
...
@Component
public class AfJjfzAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/afJjfz_appstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.jijia.fenzheng60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.jijia.fenzheng300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.jijia.fenzheng980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.jijia.fenzheng1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.jijia.fenzheng3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.jijia.fenzheng6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    public GameError doLogin(DoLoginRq req, DoLoginRs.Builder response) {
        ...
        if (userId == null) {
            LOG.error("afJjfz_appstore doLogin  userId is null");
            return GameError.SDK_LOGIN;
        }
    ...
    private boolean verifyAccount(String openid, String token) {
        try {
            ...
            if (code != 0) {
                LOG.error("afJjfz_appstore 服务端拒绝提供服务");
                return false;
            }
            LOG.error("afJjfz_appstore 调用sidInfo接口结束");
            return true;
        } catch (Exception e) {
            LOG.error("afJjfz_appstore 接口返回异常");
            e.printStackTrace();
            return false;
        }
    }
    ...
    private AnfanAccount verifyAccount(String uid, String ucid, String uuid) {
        try {
            LOG.error("afJjfz_appstore 开始调用sidInfo接口:" + serverUrl);
            ...
            LOG.error("afJjfz_appstore 待签名字符串： " + postdatasb.toString());
    ...
    public boolean verifyPay(String originSign, String openId, String sn) {
        String result = null;
        try {
            LOG.error("verifyPay of afJjfz_appstore");
            ...
            LOG.error("afJjfz_appstore signstr " + signstr);
            String sign = MD5Util.toMD5(signstr.toString());
            LOG.error("afJjfz_appstore sign " + sign);
            signstr = signstr.substring(0, signstr.lastIndexOf("&"));
            signstr = signstr + "&sign=" + sign;
            LOG.error("afJjfz_appstore 充值查询接口请求参数" + signstr);
            result = HttpUtils.sentPost(PAY_VERIRY_URL, signstr);
            JSONObject resultObject = JSONObject.fromObject(result);
            if (resultObject.getInt("code") == 0) {
                return true;
            } else {
                LOG.error("afJjfz_appstore 接口返回错误 code: " + resultObject.getInt("code") + ", " + resultObject.get("msg"));
                return false;
            }
        } catch (Exception e) {
            LOG.error("afJjfz_appstore 接口返回异常:" + result);
            e.printStackTrace();
            return false;
        }
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay of afJjfz_appstore");
        try {
            Map<String, String> params = new HashMap<String, String>();
            ...
            if (!APP_ID.equals(params.get("vid"))) {
                LOG.error("afJjfz_appstore afJjfz_appstore 充值应用ID不一致！！ ");
                return "ERROR";
            }
            ...
            if (sign.equals(originSign)) {
                if (verifyPay(originSign, params.get("open_id"), params.get("sn"))) {
                    ...
                    if (code != 0) {
                        LOG.error("afJjfz_appstore 充值发货失败！！ " + code);
                    }
                    return "SUCCESS";
                } else {
                    LOG.error(" 充值验证失败！！ ");
                ...
                LOG.error("afJjfz_appstore 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }

        } catch (Exception e) {
            LOG.error("afJjfz_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
