package com.account.plat.impl.anfan1;
...
@Component
public class Anfan1Plat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/anfan1/", "plat.properties");
    ...
    private AnfanAccount verifyAccount(String uid, String ucid, String uuid) {
        try {
            LOG.error("anfan1开始调用sidInfo接口:" + serverUrl);
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        try {
            ...
            if (sign.equalsIgnoreCase(originSign)) {
                ...
                if (code != 0) {
                    LOG.error("anfan1 充值发货失败！！ " + code);
                }
                return "SUCCESS";
            } else {
                LOG.error("anfan1 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("anfan1 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
