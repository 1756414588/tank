package com.account.plat.impl.afghgzh;
...
@Component
public class AfghgzhPlat extends PlatBase {
    // sdk server的接口地址
    private static String serverUrl = "";
    private static String serverUrl2 = "";
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/afghgzh/", "plat.properties");
        ...
        serverUrl2 = properties.getProperty("VERIRY_URL2");
    ...
    public int getPlatNo() { // // 角色与安峰老坦克互通
        return 80;
    }
    ...
    public GameError doLogin(DoLoginRq req, DoLoginRs.Builder response) {
        ...
        if (!(1 < vParam.length && vParam.length < 4)) {
            return GameError.PARAM_ERROR;
        }
        if (vParam.length == 3) {
            String uid = vParam[0];
        ...
        if (vParam.length == 2) {
            ...
            boolean flag = verifyAccount(uid, token);
            if (!flag) {
                return GameError.SDK_LOGIN;
            }
        }
    ...
    // }
    // }
    private boolean verifyAccount(String open_id, String token) {
        try {
            LOG.error("Afghgzh开始调用sidInfo接口:" + serverUrl2);
            ...
            // 对排序后的参数附加开发商签名密钥
            postdatasb.append("&sign_key=" + APP_SECRET);
            String sign = md5(postdatasb.toString().getBytes());
            LOG.error("签名值：" + sign);
            postdata += "&sign=" + sign;
            LOG.error("需要发送到服务器的数据为：" + postdata);

            String result = HttpUtils.sentPost(serverUrl2, postdata);
    ...
    private AnfanAccount verifyAccount(String uid, String ucid, String uuid) {
        try {
            LOG.error("Afghgzh开始调用sidInfo接口:" + serverUrl);
     ...
     * @param response
     * @return
     * @see com.account.plat.PlatInterface#payBack(org.springframework.web.context.request.WebRequest, java.lang.String,
     * javax.servlet.http.HttpServletResponse)
     */
    @Override
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        // TODO Auto-generated method stub
        LOG.error("pay Afghgzh");
        LOG.error("[接收到的参数]" + content);
        if (!(content.indexOf("open_id") == -1)) {
            try {
                Map<String, String> params = new HashMap<String, String>();
                ...
                if (sign.equalsIgnoreCase(originSign)) {
                    ...
                    if (code != 0) {
                        LOG.error("Afghgzh 充值发货失败！！ " + code);
                    }
                    return "SUCCESS";
                } else {
                    LOG.error("Afghgzh 签名不一致！！ " + originSign + "|" + sign);
                    return "ERROR";
                }
            } catch (Exception e) {
                LOG.error("Afghgzh 充值异常！！ " + e.getMessage());
                e.printStackTrace();
                return "ERROR";
            }
        } else {
