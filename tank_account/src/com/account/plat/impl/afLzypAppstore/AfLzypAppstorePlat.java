package com.account.plat.impl.afLzypAppstore;
...
@Component
public class AfLzypAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/afLzypAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        ...
        RECHARGE_MAP.put(1, "com.hongjing.lzyp60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.hongjing.lzyp300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.hongjing.lzyp980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.hongjing.lzyp1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.hongjing.lzyp3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.hongjing.lzyp6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    private boolean verifyAccount(String openid, String token) {
        try {
            LOG.error("AfLzypAppstore开始调用sidInfo接口:" + serverUrl);
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        try {
            ...
            if (!APP_ID.equals(params.get("vid"))) {
                LOG.error("AfLzypAppstore 充值应用ID不一致！！ ");
                return "ERROR";
            }
            ...
            if (sign.equalsIgnoreCase(originSign)) {
                ...
                if (code != 0) {
                    LOG.error("AfLzypAppstore 充值发货失败！！ " + code);
                }
                return "SUCCESS";
            } else {
                LOG.error("AfLzypAppstore 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("AfLzypAppstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
