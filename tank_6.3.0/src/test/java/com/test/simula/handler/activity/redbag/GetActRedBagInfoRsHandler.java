...
public class GetActRedBagInfoRsHandler implements ISimulaHandler {
    ...
    public void doCommand(ChannelHandlerContext ctx, BasePb.Base msg) {
        ...
        LogUtil.info("世界聊天频道红包信息：");
        for (CommonPb.RedBagChat chat : res.getWorldChatList()) {
            LogUtil.info("chat:"+chat);
        }
        LogUtil.info("军团聊天频道红包信息: ");
        for (CommonPb.RedBagChat chat : res.getPartyChatList()) {
            LogUtil.info("chat:"+chat);
        }
    }
}
