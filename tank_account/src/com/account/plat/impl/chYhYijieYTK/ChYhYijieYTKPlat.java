package com.account.plat.impl.chYhYijieYTK;
...
@Component
public class ChYhYijieYTKPlat extends PlatBase {
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chYhYijieYTK/", "plat.properties");
    ...
    private boolean verifyAccount(String[] param) {
        try {
            LOG.error("chYhYijieYTK 开始调用sidInfo接口");
            String app = param[0];
            String sdk = param[1];
            ...
            LOG.error("chYhYijieYTK 参数结果：" + sdk + "_" + app + "_" + uin + "_" + sess);

            String url = ServerUrl + "?AppID=" + AppID + "&sdk=" + sdk + "&app=" + app + "&uin=" + uin + "&sess=" + sess + "&AppKey=" + AppKey;

            LOG.error("chYhYijieYTK 请求url [{}]", url);

            String result = HttpUtils.sendGet(url, new HashMap<String, String>());

            LOG.error("chYhYijieYTK 响应结果" + result);

            if ("0".equals(result)) {
                LOG.error("chYhYijieYTK登陆成功");
                return true;
            } else {
                LOG.error("chYhYijieYTK登陆失败:" + result);
                return false;
            }
        } catch (Exception e) {
            LOG.error("chYhYijieYTK 接口返回异常:", e);
            return false;
        } finally {
            LOG.error("chYhYijieYTK 调用sidInfo接口结束");
        }
    }
...
//				
                ...
                LOG.error("afNewMjdzh1_appstore 签名不一致！！ " + checkSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("afNewMjdzh1_appstore 充值异常", e);
            return "ERROR";
        }
    }
