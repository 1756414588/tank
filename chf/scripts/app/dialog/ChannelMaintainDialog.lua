--
-- Author: Your Name
-- Date: 2017-10-14 11:49:15
--

-- 渠道问题引导弹出框

local ChannelMaintainDialog = class("ChannelMaintainDialog", function()
...
function ChannelMaintainDialog.getInstance()
	if not instance_ then
		instance_ = ChannelMaintainDialog.new()
	end
	return instance_
end

function ChannelMaintainDialog:ctor()
...
function ChannelMaintainDialog:show(msgData, callback, cancelCallback)
	...
	gdump(msgData, "ChannelMaintainDialog")

	local parent = self:getParent()

	if parent then
		gprint("[ChannelMaintainDialog] dialog added to parent already!!!")
	else
		...
		local msg = ui.newTTFLabel({text = msgData, font = FONTS, size = FONT_SIZE_MEDIUM, x = 30, y = 240, color = cc.c3b(255, 255, 255),dimensions = cc.size(self.m_bg:width() - 60,0)}):addTo(self.m_bg, 2)
...
function ChannelMaintainDialog:onExit()
	instance_ = nil
end

return ChannelMaintainDialog
