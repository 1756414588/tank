package com.account.plat.impl.chZjqytkdzAppstore;
...
@Component
public class ChZjqytkdzAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chZjqytkdzAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.zjqytkfb.60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.zjqytkfb.300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.zjqytkfb.980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.zjqytkfb.1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.zjqytkfb.3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.zjqytkfb.6480");
        MONEY_MAP.put(6, 648);
    }

    public int getPlatNo() {  // 角色与541渠道互通
        return 541;
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay chZjqytkdz_appstore ");
        LOG.error("pay chZjqytkdz_appstore  content:" + content);
        LOG.error("[开始参数]");
        try {
...
//			int money = MONEY_MAP.get(rechargeId);
            ...
            if (code != 0) {
                LOG.error("chZjqytkdz_appstore  充值发货失败！！ " + code);
                if (code == 2) {
                    return returnCode(203);
                }
            } else {
                LOG.error("chZjqytkdz_appstore  充值发货成功！！ " + code);
            }
            return returnCode(200);
        } catch (Exception e) {
            LOG.error("chCjzjtkzz_appstore  充值异常:" + e.getMessage());
            e.printStackTrace();
            return returnCode(203);
        }
    }
    ...
    private boolean verifyAccount(String userid, String token) {
        LOG.error("chZjqytkdz_appstore  开始调用sidInfo接口");
        try {
