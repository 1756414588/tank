package com.account.plat.impl.anfan;
...
@Component
public class AnfanPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/anfan/", "plat.properties");
    ...
    private boolean verifyAccount(String open_id, String token) {
        try {
            LOG.error("anfan开始调用sidInfo接口:" + serverUrl2);
        ...
        if (!(content.indexOf("open_id") == -1)) {
            try {
                ...
                if (sign.equalsIgnoreCase(originSign)) {
                    ...
                    if (code != 0) {
                        LOG.error("anfan 充值发货失败！！ " + code);
                    }
                    return "SUCCESS";
                } else {
                    LOG.error("anfan 签名不一致！！ " + originSign + "|" + sign);
                    return "ERROR";
                }
            } catch (Exception e) {
                LOG.error("anfan 充值异常！！ " + e.getMessage());
                e.printStackTrace();
                return "ERROR";
            }
        } else {
