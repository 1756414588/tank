...
function MineLayer:ctor(tileMap)
...
function MineLayer:onTick(dt)
    local curTime = ManagerTimer.getTime()

    for pos, mapData in pairs(StaffMO.mapData_) do
        if mapData.freeTime < curTime then -- 保护时间到了
            local tilePos = StaffMO.decodePosition(pos)
...
function MineLayer:getVertexZAt(tilePos)
...
function MineLayer:hasTileAt(tilePos)
...
function MineLayer:removeTileAt(tilePos)
...
function MineLayer:hasLoadAt(tilePos)
    if tilePos.x < 0 or tilePos.y < 0 or tilePos.x > (self.layerSize_.width - 1) or tilePos.y > (self.layerSize_.height - 1) then
        return true
    end

    if not self.viewContainer_[tilePos.x] then
        error("MineLayer hasLoadAt " .. tilePos.x)
    end
    local node = self.viewContainer_[tilePos.x][tilePos.y]
...
function MineLayer:reloadTileAt(tilePos)
   if tilePos.x < 0 or tilePos.x > self.layerSize_.width - 1
        or tilePos.y < 0 or tilePos.y > self.layerSize_.height - 1 then
        return
    end
    
    if not self.viewContainer_[tilePos.x] then error("MineLayer reloadTileAt AA") end

    local node = self.viewContainer_[tilePos.x][tilePos.y]

    if not node then error("MineLayer reloadTileAt BB") end
...
function MineLayer:loadTileAt(tilePos)
    if tilePos.x < 0 or tilePos.x > self.layerSize_.width - 1 or tilePos.y < 0 or tilePos.y > self.layerSize_.height - 1 then return end
    
    if not self.viewContainer_[tilePos.x] then error("MineLayer loadTileAt 11") end

    local node = self.viewContainer_[tilePos.x][tilePos.y]

    if not node then error("MineLayer loadTileAt 22") end

    node.load = true -- 加载了

    local mine = StaffBO.getMineAt(tilePos)
    ...
    local mapData = StaffMO.getMapDataAt(tilePos.x, tilePos.y)
