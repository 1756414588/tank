...
public class StaticTacticsDataMgr extends BaseDataMgr {
    ...
    private Map<Integer, Map<Integer, Map<Integer, StaticTacticsBreak>>> tacticsBreakConfig =
            new HashMap<>();
    private Map<Integer, Map<Integer, Map<Integer, StaticTacticsTankSuit>>> tacticsTankSuitConfig =
            new HashMap<>();
    ...
    private void initConfig() {
...

        {
            ...
            List<StaticTacticsTacticsRestrict> tacticsTacticsRestrictList =
                    staticDataDao.selectStaticTacticsTacticsRestrict();
        ...
        {
            Map<Integer, Map<Integer, Map<Integer, StaticTacticsBreak>>> tempTacticsBreakConfig =
                    new HashMap<>();
            ...
            for (StaticTacticsBreak s : tacticsBreakList) {
                ...
                if (!tempTacticsBreakConfig.containsKey(s.getQuality())) {
                    tempTacticsBreakConfig.put(
                            s.getQuality(), new HashMap<Integer, Map<Integer, StaticTacticsBreak>>());
                }

                if (!tempTacticsBreakConfig.get(s.getQuality()).containsKey(s.getTacticsType())) {
                    tempTacticsBreakConfig
                            .get(s.getQuality())
                            .put(s.getTacticsType(), new HashMap<Integer, StaticTacticsBreak>());
                }
        ...
        {
            Map<Integer, Map<Integer, Map<Integer, StaticTacticsTankSuit>>> tempTacticsTankSuitConfig =
                    new HashMap<>();
            List<StaticTacticsTankSuit> tacticsTankSuitkList =
                    staticDataDao.selectStaticTacticsTankSuit();
            for (StaticTacticsTankSuit s : tacticsTankSuitkList) {

                if (!tempTacticsTankSuitConfig.containsKey(s.getQuality())) {
                    tempTacticsTankSuitConfig.put(
                            s.getQuality(), new HashMap<Integer, Map<Integer, StaticTacticsTankSuit>>());
                }

                if (!tempTacticsTankSuitConfig.get(s.getQuality()).containsKey(s.getTacticsType())) {
                    tempTacticsTankSuitConfig
                            .get(s.getQuality())
                            .put(s.getTacticsType(), new HashMap<Integer, StaticTacticsTankSuit>());
                }

                tempTacticsTankSuitConfig
                        .get(s.getQuality())
                        .get(s.getTacticsType())
                        .put(s.getTankType(), s);
            }
    ...
    public StaticTacticsTankSuit getStaticTacticsTankSuitConfig(
            int quality, int tacticsType, int tankType) {
