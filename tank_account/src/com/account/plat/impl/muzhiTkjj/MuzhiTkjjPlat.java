package com.account.plat.impl.muzhiTkjj;
...
@Component
public class MuzhiTkjjPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/muzhiTkjj/", "plat.properties");
        ...
        try {
            ...
            if (mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                ...
                if (!AppID.equals(game_id)) {
                    LOG.error("muzhiTkjj 充值回调错误！！gameid: " + game_id);
                    return "success";
                }
                ...
                if (payStatus == 0) {
                    ...
                    if (code != 0) {
                        LOG.error("muzhiTkjj 充值发货失败！！ " + code);
                    } else {
                        LOG.error("muzhiTkjj 充值,发货成功");
                    }
                } else {
                    LOG.error("muzhiTkjj 充值,失败订单，跳过");
                }
            } else {
                LOG.error("muzhiTkjj 充值,签名验证失败 ");
            }
        } catch (Exception e) {
            LOG.error("muzhiTkjj 充值异常！！ ");
            LOG.error("muzhiTkjj 充值异常:" + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean verifyAccount(String userid, String usename, String sign) {
        LOG.error("muzhiTkjj 开始调用sidInfo接口");
        ...
        try {
            ...
            LOG.error("muzhiTkjj 用户验证异常:" + e.getMessage());
            e.printStackTrace();
        }
