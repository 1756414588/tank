package com.account.plat.impl.mzHszzjj_appstore;
...
@Component
public class MzHszzjjAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/mzHszzjj_appstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.lonyo.hongse_6");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.lonyo.hongse_30");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.lonyo.hongse_98");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.lonyo.hongse_198");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.lonyo.hongse_328");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.lonyo.hongse_648");
        MONEY_MAP.put(6, 648);
    }

    @Override
    public int getPlatNo() {  // 角色与501渠道互通
        return 501;
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay mzHszzjj_appstore content:" + content);
        boolean flag = false;
        ...
        try {
            if (flag) {// 拇指充值
                ...
                LOG.error("mzHszzjj_appstore content " + body);
                LOG.error("mzHszzjj_appstore签名结果 " + mysign + "|" + sign);
                ...
                if (payStatus != 0) {
                    LOG.error("mzHszzjj_appstore 充值未完成");
                    return "fail";
                }
                ...
                if (info.length < 3) {
                    LOG.error("mzHszzjj_appstore 自由参数不正确");
                    return "success";
                }
                ...
                if (code != 0) {
                    LOG.error("mzHszzjj_appstore 充值发货失败！！ " + code);
                } else {
                    LOG.error("mzHszzjj_appstore 充值发货成功！！ ");
                }
                return "success";
                ...
                LOG.error("mzHszzjj_appstore appstore 返回" + result);
                ...
                if (!product_id.equals(RECHARGE_MAP.get(rechargeId))) {
                    LOG.error("mzHszzjj_appstore rechargeId abnormal!!!");
                    return "FAILURE";
                }
            ...
            LOG.error("mzHszzjj_appstore 充值异常！！ ");
            e.printStackTrace();
            return "0|" + (System.currentTimeMillis() / 1000);
        }
    }
    ...
    private boolean verifyAccount(String userId, String usename, String sign, String time) {
        LOG.error("mzHszzjj_appstore 开始调用sidInfo接口");
        try {
            ...
            LOG.error("mzHszzjj_appstore签名原文" + signSource);
            LOG.error("mzHszzjj_appstore签名结果" + signGenerate);
            LOG.error("mzHszzjj_appstore签名传入" + sign);
