package com.account.plat.impl.afGhgzhAppstore;
...
@Component
public class AfGhgzhAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/afGhgzhAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.zhuayou.ghgzh60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.zhuayou.ghgzh300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.zhuayou.ghgzh980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.zhuayou.ghgzh1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.zhuayou.ghgzh3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.zhuayou.ghgzh6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    private AnfanAccount verifyAccount(String uid, String ucid, String uuid) {
        try {
            LOG.error("afGhgzh_appstore开始调用sidInfo接口:" + serverUrl);
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        // TODO Auto-generated method stub
        LOG.error("pay afGhgzh_appstore");
        LOG.error("[接收到的参数]" + content);
        try {
            Map<String, String> params = new HashMap<String, String>();
            ...
            if (!APP_ID.equals(params.get("vid"))) {
                LOG.error("afGhgzh_appstore 充值应用ID不一致！！ ");
                return "ERROR";
            }
            ...
            if (sign.equalsIgnoreCase(originSign)) {
                ...
                if (code != 0) {
                    LOG.error("afGhgzh_appstore 充值发货失败！！ " + code);
                }
                return "SUCCESS";
            } else {
                LOG.error("afGhgzh_appstore 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("afGhgzh_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
