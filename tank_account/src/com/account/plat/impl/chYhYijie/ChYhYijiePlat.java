package com.account.plat.impl.chYhYijie;
...
@Component
public class ChYhYijiePlat extends PlatBase {
    ...
    public void init() {
        Properties properties = loadProperties(
                "com/account/plat/impl/chYhYijie/", "plat.properties");
    ...
    private boolean verifyAccount(String[] param) {
        try {
            LOG.error("代四明的代码");
            LOG.error("chYhYijie 开始调用sidInfo接口");
            ...
            if ("0".equals(result)) {
                LOG.error("chYhYijie登陆成功");
                return true;
            } else {
                LOG.error("chYhYijie登陆失败:" + result);
                return false;
            }
        } catch (Exception e) {
            LOG.error("chYhYijie 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
