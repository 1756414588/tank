package com.account.plat.impl.chQzTkzhg_appstore;
...
@Component
public class ChQzTkzhgAppstorePlat extends PlatBase {
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chQzTkzhg_appstore/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("chQzTkzhg_appstore payBack 开始 " + content);
        try {
            ...
            LOG.error("chQzTkzhg_appstore payBack signstr:" + sb.toString());
            LOG.error("chQzTkzhg_appstore payBack sign:" + sign);
            if (!sign.equalsIgnoreCase(checkSign)) {
                LOG.error("chQzTkzhg_appstore payBack sign:md5验证失败");
                return "FAILURE";
            }
            ...
            if (code == 0 || code == 1) {
                LOG.error("chQzTkzhg_appstore 充值发货成功！！ " + code);
                return "SUCCESS";
            }
            LOG.error("chQzTkzhg_appstore 充值发货失败！！ " + code);
            return "FAILURE";


        } catch (Exception e) {
            LOG.error("chQzTkzhg_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "FAILURE";
        }
    }
