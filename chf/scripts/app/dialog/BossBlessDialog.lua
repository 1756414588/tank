...
function BlessTableView:createCellAtIndex(cell, index)
	...
	local blessLv = ActivityCenterMO.boss_["bless" .. index]
	-- 名称
	...
	if blessLv < ACTIVITY_BOSS_BLESS_MAX_LV then -- 还没有到达最高等级
		...
		local countLabel = ui.newTTFLabel({text = ActivityCenterBO.getBlessPrice(blessLv), font = G_FONT, size = FONT_SIZE_SMALL, x = itemView:getPositionX() + itemView:getContentSize().width + 5, y = itemView:getPositionY(), color = COLOR[3], align = ui.TEXT_ALIGN_CENTER}):addTo(cell)
		countLabel:setAnchorPoint(cc.p(0, 0.5))
...
function BlessTableView:onUseCallback(tag, sender)
	...
	local blessLv = ActivityCenterMO.boss_["bless" .. index]
	local take = ActivityCenterBO.getBlessPrice(blessLv)
	...
	local function gotoBuy()
		...
		ActivityCenterBO.asynBlessBossFight(doneCallback, index)
	end
...
local BossBlessDialog = class("BossBlessDialog", Dialog)

function BossBlessDialog:ctor()
	BossBlessDialog.super.ctor(self, IMAGE_COMMON .. "bg_dlg_1.png", UI_ENTER_NONE, {scale9Size = cc.size(588, 860)})
end

function BossBlessDialog:onEnter()
	BossBlessDialog.super.onEnter(self)
