...
local CoinConfirmDialog = class("CoinConfirmDialog", Dialog)

function CoinConfirmDialog:ctor(desc, okCallback, cancelCallback)
	CoinConfirmDialog.super.ctor(self, IMAGE_COMMON .. "bg_dlg_2.png", UI_ENTER_NONE, {scale9Size = cc.size(550, 360)})
...
function CoinConfirmDialog:onEnter()
	CoinConfirmDialog.super.onEnter(self)
...
function CoinConfirmDialog:setCancelBtnText(text)
	if not text then return end

	self.m_cancelBtn:setLabel(text)
end

function CoinConfirmDialog:setOkBtnText(text)
	if not text then return end
