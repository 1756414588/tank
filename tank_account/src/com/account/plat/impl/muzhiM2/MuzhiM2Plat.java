package com.account.plat.impl.muzhiM2;
...
import com.account.plat.impl.muzhiM2.Base64;
import com.account.plat.impl.muzhiM2.Rsa;
...
@Component
public class MuzhiM2Plat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/muzhiM2/", "plat.properties");
        ...
        try {
            ...
            if (mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                ...
                if (!AppID.equals(game_id)) {
                    LOG.error("mzwM2 充值回调错误！！gameid: " + game_id);
                    return "success";
                }
                ...
                if (payStatus == 0) {
                    ...
                    if (code != 0) {
                        LOG.error("mzwM2 充值发货失败！！ " + code);
                    } else {
                        LOG.error("mzwM2 充值,发货成功");
                    }
                } else {
                    LOG.error("mzwM2 充值,失败订单，跳过");
                }
            } else {
                LOG.error("mzwM2 充值,签名验证失败 ");
            }
        } catch (Exception e) {
            LOG.error("mzwM2 充值异常！！ ");
            LOG.error("mzwM2 充值异常:" + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean verifyAccount(String userid, String usename, String sign) {
        ...
        try {
            ...
            LOG.error("mzwM2 用户验证异常:" + e.getMessage());
            e.printStackTrace();
        }
