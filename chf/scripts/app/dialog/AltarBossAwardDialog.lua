...
function BossAwardTableView:ctor(size)
	...
	self.list = PartyMO.getRankAward(lv)--PartyBattleMO.getRankAward(3)
...
local AltarBossAwardDialog = class("AltarBossAwardDialog", Dialog)

function AltarBossAwardDialog:ctor()
	AltarBossAwardDialog.super.ctor(self, IMAGE_COMMON .. "bg_dlg_1.png", UI_ENTER_NONE, {scale9Size = cc.size(588, 860)})
end

function AltarBossAwardDialog:onEnter()
	AltarBossAwardDialog.super.onEnter(self)
...
function AltarBossAwardDialog:showUI()
	...
	local awardList = json.decode(altarboss.killAward) 


	local lable = ui.newTTFLabel({text = "击杀奖励", font = G_FONT, size = FONT_SIZE_MEDIUM, 
			x = 50, y = container:getContentSize().height - 80, color = COLOR[1], align = ui.TEXT_ALIGN_CENTER}):addTo(container)
	lable:setAnchorPoint(cc.p(0,0.5))
	for index=1,#awardList do
		local award = awardList[index]
		local itemView = UiUtil.createItemView(award[1], award[2])

		itemView:setPosition(80 + itemView:getContentSize().width / 2 + (index - 1) * 110,container:getContentSize().height - 140)
		itemView:setScale(0.8)
	...
	local lable = ui.newTTFLabel({text = "排名奖励", font = G_FONT, size = FONT_SIZE_MEDIUM, 
			x = 50, y = container:getContentSize().height - 230, color = COLOR[1], align = ui.TEXT_ALIGN_CENTER}):addTo(container)
	lable:setAnchorPoint(cc.p(0,0.5))
	local view = BossAwardTableView.new(cc.size(526, 728-180)):addTo(container)
