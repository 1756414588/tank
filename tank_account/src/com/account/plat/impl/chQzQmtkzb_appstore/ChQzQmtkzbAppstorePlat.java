package com.account.plat.impl.chQzQmtkzb_appstore;
...
@Component
public class ChQzQmtkzbAppstorePlat extends PlatBase {
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chQzQmtkzb_appstore/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("chQzQmtkzb_appstore payBack 开始 " + content);
        try {
            ...
            LOG.error("chQzQmtkzb_appstore payBack signstr:" + sb.toString());
            LOG.error("chQzQmtkzb_appstore payBack sign:" + sign);
            if (!sign.equalsIgnoreCase(checkSign)) {
                LOG.error("chQzQmtkzb_appstore payBack sign:md5验证失败");
                return "FAILURE";
            }
            ...
            if (code == 0 || code == 1) {
                LOG.error("chQzQmtkzb_appstore 充值发货成功！！ " + code);
                return "SUCCESS";
            }
            LOG.error("chQzQmtkzb_appstore 充值发货失败！！ " + code);
            return "FAILURE";


        } catch (Exception e) {
            LOG.error("chQzQmtkzb_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "FAILURE";
        }
    }
