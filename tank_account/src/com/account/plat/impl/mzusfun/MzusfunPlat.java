package com.account.plat.impl.mzusfun;
...
@Component
public class MzusfunPlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/mzusfun/", "plat.properties");
    ...
    private void initRechargeMap() {
        MONEY_MAP.put(1, 30);
        PRICE_MAP.put(30.00, 30);

        MONEY_MAP.put(2, 68);
        PRICE_MAP.put(68.00, 68);

        MONEY_MAP.put(3, 128);
        PRICE_MAP.put(128.00, 128);

        MONEY_MAP.put(4, 328);
        PRICE_MAP.put(328.00, 328);

        MONEY_MAP.put(5, 648);
        PRICE_MAP.put(648.00, 648);

        PACK_PRICE_MAP.put(6.00, 1);

        PACK_PRICE_MAP.put(12.00, 2);

        PACK_PRICE_MAP.put(30.00, 3);

        PACK_PRICE_MAP.put(68.00, 4);

        PACK_PRICE_MAP.put(128.00, 5);

        PACK_PRICE_MAP.put(328.00, 6);

        PACK_PRICE_MAP.put(648.00, 7);
        ...
        try {
            ...
            if (mysign.equals(sign)) {// 签名正确,做业务逻辑处理
                ...
                if (!AppID.equals(game_id)) {
                    LOG.error("mzusfun 充值回调错误！！gameid: " + game_id);
                    return "success";
                }
                ...
                if (payStatus == 0) {
                    ...
                    if (rechargeId > 0 && PRICE_MAP.get(price).intValue() != MONEY_MAP.get(rechargeId).intValue()) {
                        LOG.error("mzusfun 商品金额不匹配 充值发货失败  ！！ ");
                        return "fail";
                    }
                    ...
                    if (code != 0) {
                        LOG.error("mzusfun 充值发货失败！！ " + code);
                    } else {
                        LOG.error("mzusfun 充值,发货成功");
                    }
                } else {
                    LOG.error("mzusfun 充值,失败订单，跳过");
                }
            } else {
                LOG.error("mzusfun 充值,签名验证失败 ");
            }
        } catch (Exception e) {
            LOG.error("mzusfun 充值异常！！ ");
            LOG.error("mzusfun 充值异常:" + e.getMessage());
            e.printStackTrace();
        }
    ...
    private boolean verifyAccount(String userid, String sign) {
        LOG.error("mzusfun 开始调用sidInfo接口");
        // String signSource = usename + AppKey;// 组装签名原文
        // String signGenerate = Rsa.getMD5(signSource).toLowerCase();

        try {
            String signSource = userid + AppKey;
            ...
            LOG.error("mzusfun 用户验证异常:" + e.getMessage());
            e.printStackTrace();
        }
