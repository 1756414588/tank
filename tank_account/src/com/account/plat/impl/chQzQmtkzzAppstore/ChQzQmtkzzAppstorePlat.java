package com.account.plat.impl.chQzQmtkzzAppstore;
...
@Component
public class ChQzQmtkzzAppstorePlat extends PlatBase {
    ...
    private boolean verifyAccount(String[] param) {
        try {
            LOG.error("chQzQmtkzzAppstore 开始调用sidInfo接口");
            ...
            if (LOGIN_STATUS_VALUE_SUCCESS.equals(status)) {
                LOG.error("chQzQmtkzzAppstore登陆成功 ");
                return true;
            } else if (LOGIN_STATUS_VALUE_NOLOGIN.equals(status)) {
                LOG.error("chQzQmtkzzAppstore登陆失败,用户未登录:" + result);
                return false;
            } else if (LOGIN_STATUS_VALUE_ERROR.equals(status)) {
                LOG.error("chQzQmtkzzAppstore登陆失败,参数错误:" + result);
                return false;
            } else {
                LOG.error("chQzQmtkzzAppstore登陆失败,未知错误:" + result);
                return false;
            }
        } catch (Exception e) {
            LOG.error("chQzQmtkzzAppstore 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error(" chQzQmtkzzAppstore payBack开始");
        try {
            ...
            if (sign.equalsIgnoreCase(checkSign)) {
                ...
                if (code != 0) {
                    LOG.error("chQzQmtkzzAppstore 充值发货失败！！ " + code);
                }
                return "success";

            } else {
                // afNewMjdzh1_appstore 签名不一致！！ 2c461a02105b45922989a515017aa7f0|286cfafa5a9324f2a80518b60a205b6e
                LOG.error("chQzQmtkzzAppstore 签名不一致！！ " + checkSign + "|" + sign);
                return "errorSign";
            }
        } catch (Exception e) {
            LOG.error("chQzQmtkzzAppstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "error";
        }
    }
