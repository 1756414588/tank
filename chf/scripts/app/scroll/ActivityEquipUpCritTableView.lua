...
local ActivityEquipUpCritTableView = class("ActivityEquipUpCritTableView", TableView)

--
function ActivityEquipUpCritTableView:ctor(size, activityId)
	ActivityEquipUpCritTableView.super.ctor(self, size, SCROLL_DIRECTION_VERTICAL)
...
function ActivityEquipUpCritTableView:numberOfCells()
	return #self.m_activityContent.items + 1
end

function ActivityEquipUpCritTableView:cellSizeForIndex(index)
	return self.m_cellSize
end

function ActivityEquipUpCritTableView:createCellAtIndex(cell, index)
	ActivityEquipUpCritTableView.super.createCellAtIndex(self, cell, index)
	...
	if index == 1 then
		local title = ui.newTTFLabel({text = CommonText[499][1], font = G_FONT, size = FONT_SIZE_SMALL, x = 40, y = titleBg:getContentSize().height / 2, align = ui.TEXT_ALIGN_CENTER}):addTo(titleBg)
		title:setAnchorPoint(cc.p(0, 0.5))

		-- local configs = {{name = "t_part", desc = CommonText[476][4]}, {name = "t_part_refit", desc = CommonText[476][5]}}
		-- local config = configs[index]

		local itemView = display.newSprite("image/item/" .. "t_equipup" .. ".jpg"):addTo(cell)
		...
		local name = ui.newTTFLabel({text = CommonText[499][2], font = G_FONT, size = FONT_SIZE_LIMIT, x = itemView:getPositionX(), y = 30, color = COLOR[1], align = ui.TEXT_ALIGN_CENTER}):addTo(cell)
	else
...
function ActivityEquipUpCritTableView:onBuyCallback(tag, sender)
...
return ActivityEquipUpCritTableView