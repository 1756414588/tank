...
public class StaticFortressDataMgr extends BaseDataMgr {
  ...
  // id ： level : attr
  private Map<Integer, Map<Integer, StaticFortressAttr>> fortressAttrMap =
      new HashMap<Integer, Map<Integer, StaticFortressAttr>>();

  private Map<Integer, StaticFortressJob> fortressJobMap =
      new HashMap<Integer, StaticFortressJob>();

  private List<Integer> attrIdList;

  @Override
  public void init() {
    sufferJifenMap = staticDataDao.selectFortressSufferJifenMap();
    attrList = staticDataDao.selectFortressAttr();
    ...
    attrIdList = new ArrayList<Integer>(fortressAttrMap.keySet());

    fortressJobMap = staticDataDao.selectFortressJob();
  }
