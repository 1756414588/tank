...
local ActivityLocalConfirmDialog = class("ActivityLocalConfirmDialog", Dialog)

function ActivityLocalConfirmDialog:ctor(desc, activitykey, defaultState, okCallback, cancelCallback)
	ActivityLocalConfirmDialog.super.ctor(self, IMAGE_COMMON .. "bg_dlg_2.png", UI_ENTER_NONE, {scale9Size = cc.size(550, 360)})
...
function ActivityLocalConfirmDialog:onEnter()
	ActivityLocalConfirmDialog.super.onEnter(self)
	...
	checkBox:setChecked(self._state)
	...
	local function onOkCallback(tag, sender)
		...
		self:doCheck()
		self:pop()
		if self.m_okCallback then self.m_okCallback(self._state ~= self.defaultState) end
	end
...
function ActivityLocalConfirmDialog:setCancelBtnText(text)
	if not text then return end

	self.m_cancelBtn:setLabel(text)
end

function ActivityLocalConfirmDialog:setOkBtnText(text)
	if not text then return end

	self.m_okBtn:setLabel(text)
end

function ActivityLocalConfirmDialog:doCheck()
	ActivityCenterMO.UseActivityLoaclRecordInfo(self.activitykey,self._state)
end

return ActivityLocalConfirmDialog
