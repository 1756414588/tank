...
local WeaponryChangDialog = class("WeaponryChangDialog", Dialog)

local movieTime = 0.3
local movieTimeInterval = 0.01

function WeaponryChangDialog:ctor(uiEnter,openColor)
	uiEnter = uiEnter or UI_ENTER_FADE_IN_GATE
	WeaponryChangDialog.super.ctor(self, IMAGE_COMMON .. "info_bg_84.png", uiEnter, {scale9Size = cc.size(588, 860)})
	self.openColor = openColor or 1
end

function WeaponryChangDialog:onEnter()
	WeaponryChangDialog.super.onEnter(self)
    --self:hasCloseButton(false)
	self:setUI()
	
end

function WeaponryChangDialog:setUI()
...
function WeaponryChangDialog:selectHeroHandle()
	self.m_view_type = self.equipData[self.selectHero]
    for i = 1, #self.equipData do
...
function WeaponryChangDialog:turnHandler(tag, sender)
...
function WeaponryChangDialog:playMovie(type)
...
function WeaponryChangDialog:updateView()
end

function WeaponryChangDialog:update(dt)
	
end

function WeaponryChangDialog:equipHandler(tag,sender)
    self:pop()
end

function WeaponryChangDialog:onExit()
