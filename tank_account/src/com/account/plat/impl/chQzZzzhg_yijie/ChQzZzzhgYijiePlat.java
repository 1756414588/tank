package com.account.plat.impl.chQzZzzhg_yijie;
...
@Component
public class ChQzZzzhgYijiePlat extends PlatBase {
    ...
    public void init() {
        Properties properties = loadProperties(
                "com/account/plat/impl/chQzZzzhg_yijie/", "plat.properties");
    ...
    public GameError doLogin(DoLoginRq req, DoLoginRs.Builder response) {
        ...
        if (!req.hasSid() || !req.hasBaseVersion() || !req.hasVersion()
                || !req.hasDeviceNo()) {
            LOG.error("chQzZzzhg_yijie GameError.PARAM_ERROR");
    ...
    private boolean verifyAccount(String[] param) {
        try {
            LOG.error("chQzZzzhg_yijie 开始调用sidInfo接口");
            ...
            LOG.error("chQzZzzhg_yijie 参数结果：" + sdk + "_" + app + "_" + uin + "_" + sess);

            String url = ServerUrl + "?AppID=" + AppID + "&sdk=" + sdk + "&app=" + app + "&uin=" + uin + "&sess=" + sess + "&AppKey=" + AppKey;

            LOG.error("chQzZzzhg_yijie 请求url" + url);

            String result = HttpUtils.sendGet(url, new HashMap<String, String>());

            LOG.error("chQzZzzhg_yijie 响应结果" + result);

            if ("0".equals(result)) {
                LOG.error("chQzZzzhg_yijie 登陆成功");
                return true;
            } else {
                LOG.error("chQzZzzhg_yijie 登陆失败:" + result);
                return false;
            }
        } catch (Exception e) {
            LOG.error("chQzZzzhg_yijie 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
            LOG.error("chQzZzzhg_yijie 调用sidInfo接口结束");
        }
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        try {
            ...
            // 打印参数名和对应的参数值
            LOG.error("chQzZzzhg_yijie signstr:" + signstr);//signstr:app=0348ba885edf4f62&cbi=1_65405259_1503383479569_B101363CA00933D2&ct=1503383500000&fee=1000&pt=1503383479450&sdk=b101363ca00933d2&ssid=38780360&st=1&tcd=565efae8545a435aadc7f47ffcbfc2e1&uid=1142791&ver=1&AppKey=7UO9WFQACAZVQXXMYK9IJWX43ANG6F8X

            LOG.error("chQzZzzhg_yijie sign:" + sign);//sign:286cfafa5a9324f2a80518b60a205b6e
...
//
                ...
                if (code != 0) {
                    LOG.error("chQzZzzhg_yijie 充值发货失败！！ " + code);
                }
                return "SUCCESS";

            } else {
                LOG.error("chQzZzzhg_yijie 签名不一致！！ " + checkSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("chQzZzzhg_yijie 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
