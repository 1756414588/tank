package com.account.plat.impl.chYhYijie2;
...
@Component
public class ChYhYijie2Plat extends PlatBase {
    ...
    public void init() {
        Properties properties = loadProperties(
                "com/account/plat/impl/chYhYijie2/", "plat.properties");
    ...
    public GameError doLogin(DoLoginRq req, DoLoginRs.Builder response) {
        ...
        if (!req.hasSid() || !req.hasBaseVersion() || !req.hasVersion()
                || !req.hasDeviceNo()) {
            LOG.error("chYhYijie2 GameError.PARAM_ERROR");
        ...
        if (authorityRs != GameError.OK) {
            LOG.error(JSON.toJSONString(authorityRs));
            return authorityRs;
        }
    ...
    private boolean verifyAccount(String[] param) {
        try {
            LOG.error("chYhYijie2 开始调用sidInfo接口");
            ...
            if ("0".equals(result)) {
                LOG.error("chYhYijie2登陆成功");
                return true;
            } else {
                LOG.error("chYhYijie2登陆失败:" + result);
                return false;
            }
        } catch (Exception e) {
            LOG.error("chYhYijie2 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        } finally {
...
//				
                ...
                if (code != 0) {
                    LOG.error("chYhYijie2 充值发货失败！！ " + code);
                }
                return "SUCCESS";

            } else {
                LOG.error("chYhYijie2 签名不一致！！ " + checkSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("chYhYijie2 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
