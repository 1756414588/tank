package com.account.plat.impl.chzzzhgAppstore;
...
@Component
public class ChzzzhgAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chzzzhgAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.zzzhg2.60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.zzzhg2.300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.zzzhg2.980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.zzzhg2.1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.zzzhg2.3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.zzzhg2.6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay chzzzhg_appstore");
        LOG.error("pay chzzzhg_appstore content:" + content);
        LOG.error("[开始参数]");
        try {
...
//			int money = MONEY_MAP.get(rechargeId);
            ...
            if (code != 0) {
                LOG.error("chzzzhg_appstore 充值发货失败！！ " + code);
                if (code == 2) {
                    return returnCode(203);
                }
            } else {
                LOG.error("chzzzhg_appstore 充值发货成功！！ " + code);
            }
            return returnCode(200);
        } catch (Exception e) {
            LOG.error("chzzzhg_appstore 充值异常:" + e.getMessage());
            e.printStackTrace();
            return returnCode(203);
        }
    }
    ...
    private boolean verifyAccount(String userid, String token) {
        LOG.error("chzzzhg_appstore 开始调用sidInfo接口");
        try {
