package com.account.plat.impl.afNewAppstore;
...
@Component
public class afNewAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/afNewAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        ...
        RECHARGE_MAP.put(1, "com.tank.tqdkn60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.tank.tqdkn300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.tank.tqdkn980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.tank.tqdkn1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.tank.tqdkn3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.tank.tqdkn6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    private boolean verifyAccount(String openid, String token) {
        try {
            LOG.error("afNew_appstore开始调用sidInfo接口:" + serverUrl);
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        ...
        try {
            ...
            if (!APP_ID.equals(params.get("vid"))) {
                LOG.error("afNew_appstore 充值应用ID不一致！！ ");
                return "ERROR";
            }
            ...
            if (sign.equalsIgnoreCase(originSign)) {
                ...
                if (code != 0) {
                    LOG.error("afNew_appstore 充值发货失败！！ " + code);
                }
                return "SUCCESS";
            } else {
                LOG.error("afNew_appstore 签名不一致！！ " + originSign + "|" + sign);
                return "ERROR";
            }
        } catch (Exception e) {
            LOG.error("afNew_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "ERROR";
        }
    }
