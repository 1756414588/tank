...
public class VerifyNew {
    ...
    public static PublicKey loadPublicKeyByStr() throws Exception {
        try {
            ...
            int count = 0;
            for (int i = 0; i < ChQzZzzhgXiaoQiPlat.getPublicKey().length(); ++i) {
                if (count < 64) {
                    publicKeyStr += ChQzZzzhgXiaoQiPlat.getPublicKey().charAt(i);
                    count++;
                } else {
                    publicKeyStr += ChQzZzzhgXiaoQiPlat.getPublicKey().charAt(i) + "\r\n";
                    count = 0;
    ...
    public static String verify(Map<String, String> params, String sign) throws Exception {

        String sourceStr = buildHttpQueryNoEncode(params);
        //验签
        if (!verifySign(sourceStr, sign, loadPublicKeyByStr())) {

            return "sign_data_verify_failed";
        }
        return null;
    }

    public static Map<String, String> decode(Map<String, String> params) throws Exception {
        //解密
        String decryptData = new String(publicKeyDecrypt(loadPublicKeyByStr(), baseDecode(params.get("encryp_data"))));
        /*************************************************************
