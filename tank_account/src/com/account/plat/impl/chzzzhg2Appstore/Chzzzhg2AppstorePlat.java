package com.account.plat.impl.chzzzhg2Appstore;
...
@Component
public class Chzzzhg2AppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chzzzhg2Appstore/", "plat.properties");
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay chzzzhg2_appstore");
        LOG.error("pay chzzzhg2_appstore content:" + content);
        LOG.error("[开始参数]");
        try {
...
//			int money = MONEY_MAP.get(rechargeId);
            ...
            if (code != 0) {
                LOG.error("chzzzhg2_appstore 充值发货失败！！ " + code);
                if (code == 2) {
                    return returnCode(203);
                }
            } else {
                LOG.error("chzzzhg2_appstore 充值发货成功！！ " + code);
            }
            return returnCode(200);
        } catch (Exception e) {
            LOG.error("chzzzhg2_appstore 充值异常:" + e.getMessage());
            e.printStackTrace();
            return returnCode(203);
        }
    }
    ...
    private boolean verifyAccount(String userid, String token) {
        LOG.error("chzzzhg2_appstore 开始调用sidInfo接口");
        try {
