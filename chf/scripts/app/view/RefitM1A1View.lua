...
local RefitM1A1ResTableView = class("RefitM1A1ResTableView", TableView)

-- tankId: 需要进行改装的tank
function RefitM1A1ResTableView:ctor(size, tankFormula)
	RefitM1A1ResTableView.super.ctor(self, size, SCROLL_DIRECTION_VERTICAL)

	self.m_cellSize = cc.size(size.width, 80)
	self.m_cellNum = 5
	self.m_tankDB = TankMO.queryTankById(tankFormula.from)
	-- 改装到的坦克
...
function RefitM1A1ResTableView:numberOfCells()
	return #self.m_assistItem
end

function RefitM1A1ResTableView:cellSizeForIndex(index)
	return self.m_cellSize
end

function RefitM1A1ResTableView:createCellAtIndex(cell, index)
	RefitM1A1ResTableView.super.createCellAtIndex(self, cell, index)
...
function RefitM1A1ResTableView:updateCell(cell, cellIndex)
...
local RefitM1A1View = class("RefitM1A1View", function()
...
function RefitM1A1View:showUI()
	local container = self

	local tankDB = TankMO.queryTankById(self.m_tankFormula.from)
	self.m_tank = tankDB
	local refitTank = TankMO.queryTankById(self.m_tankFormula.to)
	...
	local view = RefitM1A1ResTableView.new(cc.size(resBg:getContentSize().width, 278), self.m_tankFormula):addTo(resBg)
	...
	self.m_minNum = 1
	self.m_maxNum = ActivityCenterBO.getMaxRefitM1A1Num(self.m_tankFormula)
	...
	local returnBtn = MenuButton.new(normal, selected, nil, function() ManagerSound.playNormalButtonSound(); self:dispatchEvent({name = "REFIT_M1A1_RETURN_EVENT"}) end):addTo(container)
...
function RefitM1A1View:showSlider()
...
function RefitM1A1View:onResUpdate()
	-- local maxNum = TankBO.getMaxRefitNum(self.m_tankId)
	local maxNum = ActivityCenterBO.getMaxRefitM1A1Num(self.m_tankFormula)
...
function RefitM1A1View:onReduceCallback(tag, sender)
	ManagerSound.playNormalButtonSound()
...
function RefitM1A1View:onAddCallback(tag, sender)
	ManagerSound.playNormalButtonSound()
	self.m_settingNum = self.m_settingNum + 1
...
function RefitM1A1View:onSlideCallback(event)
	local value = event.value - event.value % 1
	-- -- gprint("RefitM1A1View value:", value)
	self.m_settingNum = value
	self.m_numLabel:setString(self.m_settingNum)
	self.m_resTablView:setCurProductNum(self.m_settingNum)
end

function RefitM1A1View:onProductCallback(tag, sender)
	ManagerSound.playNormalButtonSound()
	
	if ActivityCenterBO.getMaxRefitM1A1Num(self.m_tankFormula) == 0 then
		Toast.show(CommonText[368])
		return
	end
	self:dispatchEvent({name = "ARMY_M1A1_REFIT_EVENT", tankFormula = self.m_tankFormula, count = self.m_settingNum})
end

return RefitM1A1View
