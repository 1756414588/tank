--
-- Author: Your Name
-- Date: 2017-09-25 21:05:35
--

local Dialog = require("app.dialog.Dialog")
local FreshConfirmDialog = class("FreshConfirmDialog", Dialog)

function FreshConfirmDialog:ctor(id,desc, okCallback, cancelCallback)
	FreshConfirmDialog.super.ctor(self, IMAGE_COMMON .. "bg_dlg_2.png", UI_ENTER_NONE, {scale9Size = cc.size(550, 360)})
...
function FreshConfirmDialog:onEnter()
	FreshConfirmDialog.super.onEnter(self)
	...
	local function onCheckedChanged(sender, isChecked)
		...
		if self.m_id == 1 then
			ActivityCenterMO.HysperTip_ = false
		elseif self.m_id == 2 then
			ActivityCenterMO.HysperExcTip_  =false
		elseif self.m_id == 3 then
			ActivityCenterMO.HysperExchange_ = false
		end
	...
	tip:setPosition(self:getBg():getContentSize().width / 2 + 60, 155)
...
function FreshConfirmDialog:setCancelBtnText(text)
	if not text then return end

	self.m_cancelBtn:setLabel(text)
end

function FreshConfirmDialog:setOkBtnText(text)
	if not text then return end

	self.m_okBtn:setLabel(text)
end

return FreshConfirmDialog
