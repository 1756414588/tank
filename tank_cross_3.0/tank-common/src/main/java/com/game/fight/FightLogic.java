...
public class FightLogic {
    ...
    public FightLogic(Fighter attacker, Fighter defencer, int firstStrategy, boolean recordFlag) {
        GameContext.getAc().getBean(TacticsService.class).calTacticsRestrictAttribute(attacker, defencer);
        ...
        if (firstStrategy == FirstActType.ATTACKER) {
            ...
            if (attacker.firstValue > defencer.firstValue || (attacker.firstValue == defencer.firstValue && RandomHelper.isHitRangeIn100(50))) {
                first = attacker;
                second = defencer;
            } else {
                first = defencer;
    ...
    public FightLogic(Fighter attacker, Fighter defencer, int roundNum, CompetGroup fightPair, boolean recordFlag) {
        GameContext.getAc().getBean(TacticsService.class).calTacticsRestrictAttribute(attacker, defencer);
        attacker.isAttacker = true;
        this.recordFlag = recordFlag;
        if (recordFlag) {
        ...
        if (attacker.firstValue > defencer.firstValue) {
            first = attacker;
            second = defencer;
        } else if (attacker.firstValue < defencer.firstValue) {
            first = defencer;
            second = attacker;
        } else {
            if (roundNum == 1) {
                ...
                // 第一局后手的先手
                CompteRound cr1 = fightPair.getMap().get(1);
                if (cr1.getWhoFist() == 1) {
                    first = defencer;
                    second = attacker;
                } else {
                ...
                // 前两场坦克损失少的先手
                CompteRound cr1 = fightPair.getMap().get(1);
                CompteRound cr2 = fightPair.getMap().get(2);
                int attackDesNum = cr1.getAttackDestroyNum() + cr2.getAttackDestroyNum();
                int defenceDesNum = cr1.getDenfenceDestroyNum() + cr2.getDenfenceDestroyNum();

                if (attackDesNum > defenceDesNum) {
                    first = defencer;
                    second = attacker;
                } else {
                    first = attacker;
    ...
    private void packSecretWeapon(Fighter attacker, Fighter defencer) {
        // 秘密武器信息
        if (recordData != null) {
            int secretWeaponId = attacker.getHighestOpenSecretWeapon();
            if (secretWeaponId > 0) {
                CommonPb.FormExt.Builder pbExt = CommonPb.FormExt.newBuilder();
                pbExt.addWeaponId(secretWeaponId);
                recordData.setFormExtA(pbExt);
            }
        }

        if (recordData != null) {
            int secretWeaponId = defencer.getHighestOpenSecretWeapon();
            if (secretWeaponId > 0) {
                CommonPb.FormExt.Builder pbExt = CommonPb.FormExt.newBuilder();
                pbExt.addWeaponId(secretWeaponId);
                recordData.setFormExtB(pbExt);
            }
        }
    }
    ...
    public void __divisiveFighter(Fighter fighter, List<List<Integer>> param, List<List<Integer>> attr) {
        for (List<Integer> l : param) {
            int p = l.get(1);
            int count = l.get(2);
            StaticTank staticTank = GameContext.getAc().getBean(StaticTankDataMgr.class).getStaticTank(p);
    ...
    protected void printForce(Force[] targets) {
        for (int i = 0; i < targets.length; i++) {
            ...
            if (force != null) {
                LogUtil.info("force " + i + "|tankid:" + force.staticTank.getTankId() + "|key:" + force.key + "|pos:" + force.pos + "|count:" + force.count);
            } else {
    ...
    private void addSkillEffectImmune(Fighter fighter) {
        ...
        fighter.isAnxin = true;
        StaticHeroDataMgr staticHeroDataMgr = GameContext.getAc().getBean(StaticHeroDataMgr.class);
