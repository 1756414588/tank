package com.account.plat.impl.chhjfcAppstore;
...
@Component
public class ChhjfcAppstorePlat extends PlatBase {
    // sdk server的接口地址
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chhjfcAppstore/", "plat.properties");
    ...
    private void initRechargeMap() {
        RECHARGE_MAP.put(1, "com.qzkj.hjfc_60");
        MONEY_MAP.put(1, 6);

        RECHARGE_MAP.put(2, "com.qzkj.hjfc_300");
        MONEY_MAP.put(2, 30);

        RECHARGE_MAP.put(3, "com.qzkj.hjfc_980");
        MONEY_MAP.put(3, 98);

        RECHARGE_MAP.put(4, "com.qzkj.hjfc_1980");
        MONEY_MAP.put(4, 198);

        RECHARGE_MAP.put(5, "com.qzkj.hjfc_3280");
        MONEY_MAP.put(5, 328);

        RECHARGE_MAP.put(6, "com.qzkj.hjfc_6480");
        MONEY_MAP.put(6, 648);
    }
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("pay ChhjfcAppstore ");
        LOG.error("pay ChhjfcAppstore  content:" + content);
        LOG.error("[开始参数]");
        try {
...
//			int money = MONEY_MAP.get(rechargeId);
            ...
            if (code != 0) {
                LOG.error("ChhjfcAppstore  充值发货失败！！ " + code);
                if (code == 2) {
                    return returnCode(203);
                }
            } else {
                LOG.error("ChhjfcAppstore  充值发货成功！！ " + code);
            }
            return returnCode(200);
        } catch (Exception e) {
            LOG.error("ChhjfcAppstore  充值异常:" + e.getMessage());
            e.printStackTrace();
            return returnCode(203);
        }
    }
    ...
    private boolean verifyAccount(String userid, String token) {
        LOG.error("ChhjfcAppstore  开始调用sidInfo接口");
        try {
