package com.account.plat.impl.chQzZjldzAppstore;
...
@Component
public class ChQzZjldzAppstorePlat extends PlatBase {
    ...
    @PostConstruct
    public void init() {
        Properties properties = loadProperties("com/account/plat/impl/chQzZjldzAppstore/", "plat.properties");
    ...
    private boolean verifyAccount(String accessToken) {
        try {
            ...
            LOG.error("chQzZjldz_appstore str " + str);
            ...
            LOG.error("chqzzjldz_appstore 接口返回异常:" + e.getMessage());
            e.printStackTrace();
            return false;
        }
        return false;
    ...
    public String payBack(WebRequest request, String content, HttpServletResponse response) {
        LOG.error("chQzZjldz_appstore payBack 开始 " + content);
        ...
        try {
            ...
            while (iterator.hasNext()) {
                ...
                LOG.error("chQzZjldz_appstore " + paramName + ":" + request.getParameter(paramName));
            ...
            LOG.error("chQzZjldz_appstore payBack signstr:" + sb.toString());
            LOG.error("chQzZjldz_appstore payBack sign:" + sign);
            if (!sign.equalsIgnoreCase(checkSign)) {
                LOG.error("chQzZjldz_appstore payBack sign:md5验证失败");
                return "FAILURE";
            }
            ...
            if (code == 0 || code == 1) {
                LOG.error("chQzZjldz_appstore 充值发货成功！！ " + code);
                return "0";
            }
            LOG.error("chQzZjldz_appstore 充值发货失败！！ " + code);
            return "FAILURE";

        } catch (Exception e) {
            LOG.error("chQzZjldz_appstore 充值异常！！ " + e.getMessage());
            e.printStackTrace();
            return "FAILURE";
        }
    }
